...
public final class ZipkinProxyService extends AbstractHttpService {
    ...
    public ZipkinProxyService(String[] zipkinServices) throws Exception {
        final List<Endpoint> endpoints = Stream.of(zipkinServices).map(URI::create).map(URI::getAuthority).map(Endpoint::parse).collect(toList());
    ...
    private static WebClient newLoadBalancingClient(EndpointGroup zipkinGroup) {
        try {
            return WebClient.builder(SessionProtocol.HTTP, zipkinGroup).decorator(LoggingClient.newDecorator()).build();
