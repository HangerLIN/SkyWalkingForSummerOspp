...
public class ProfileTaskHandler extends AbstractKafkaHandler {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ProfileTaskHandler.class);
    ...
    public ProfileTaskHandler(ModuleManager manager, KafkaFetcherConfig config) {
        ...
        histogram = metricsCreator.createHistogramMetric("profile_task_in_latency", "The process latency of profile task", new Keys("protocol"), new Values("kafka"));
        errorCounter = metricsCreator.createCounter("profile_task_analysis_error_count", "The error number of profile task process", new Keys("protocol"), new Values("kafka"));
    }
    ...
    public void handle(final ConsumerRecord<String, Bytes> record) {
        try (Timer ignored = histogram.createTimer()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Fetched a thread snapshot[{}] from task[{}] reported", snapshot.getTraceSegmentId(), snapshot.getTaskId());
            }
