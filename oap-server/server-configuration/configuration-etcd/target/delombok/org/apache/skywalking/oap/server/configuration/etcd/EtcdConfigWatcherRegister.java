...
public class EtcdConfigWatcherRegister extends FetchingConfigWatcherRegister {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(EtcdConfigWatcherRegister.class);
    private final KV client;

    public EtcdConfigWatcherRegister(EtcdServerSettings setting) {
        super(setting.getPeriod());
        ClientBuilder builder = Client.builder().authority(setting.getAuthority()).endpoints(setting.getEndpointArray());
        ...
        if (setting.isAuthentication()) {
            builder.user(ByteSequence.from(setting.getUser(), Charset.defaultCharset())).password(ByteSequence.from(setting.getPassword(), Charset.defaultCharset()));
        }
    ...
    public Optional<GroupConfigTable> readGroupConfig(final Set<String> keys) {
        ...
        keys.forEach(key -> {
            ...
            String groupKey = key + "/";
            GetOption option = GetOption.newBuilder().withPrefix(ByteSequence.from(groupKey, Charset.defaultCharset())).build();
            try {
