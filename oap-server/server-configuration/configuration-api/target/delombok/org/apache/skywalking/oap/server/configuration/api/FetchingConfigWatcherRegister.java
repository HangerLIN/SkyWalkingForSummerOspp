...
public abstract class FetchingConfigWatcherRegister extends ConfigWatcherRegister {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(FetchingConfigWatcherRegister.class);
    ...
    public synchronized void registerConfigChangeWatcher(ConfigChangeWatcher watcher) {
        ...
        if (singleConfigChangeWatcherRegister.containsKey(holder.getKey()) || groupConfigChangeWatcherRegister.containsKey(holder.getKey())) {
    ...
    public void start() {
        ...
        Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(new RunnableWithExceptionProtection(this::configSync, t -> log.error("Sync config center error.", t)), 0, syncPeriod, TimeUnit.SECONDS);
    }
    ...
    private void singleConfigsSync() {
        ...
        configTable.ifPresent(config -> {
            config.getItems().forEach(item -> {
                ...
                if (holder == null) {
                    log.warn("Config {} from configuration center, doesn\'t match any WatchType.SINGLE watcher, ignore.", itemName);
                    return;
                }
            ...
            if (log.isTraceEnabled()) {
                log.trace("Current configurations after the sync." + LINE_SEPARATOR + singleConfigChangeWatcherRegister.toString());
            }
        });
    }
    ...
    private void groupConfigsSync() {
        ...
        groupConfigTable.ifPresent(config -> {
            config.getGroupItems().forEach(groupConfigItems -> {
                ...
                if (holder == null) {
                    log.warn("Config {} from configuration center, doesn\'t match any WatchType.GROUP watcher, ignore.", groupConfigItemName);
                    return;
                }
            ...
            if (log.isTraceEnabled()) {
                log.trace("Current configurations after the sync." + LINE_SEPARATOR + groupConfigChangeWatcherRegister.toString());
            }
        });
    }
    ...
    static class Register {
        ...
        public String toString() {
            ...
            register.forEach((key, holder) -> {
                ConfigChangeWatcher watcher = holder.getWatcher();
                registerTableDescription.append("key:").append(key).append("    module:").append(watcher.getModule()).append("    provider:").append(watcher.getProvider().name());
