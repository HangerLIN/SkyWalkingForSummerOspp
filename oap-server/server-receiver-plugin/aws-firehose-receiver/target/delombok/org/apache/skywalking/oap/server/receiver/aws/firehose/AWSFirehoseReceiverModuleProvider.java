...
public class AWSFirehoseReceiverModuleProvider extends ModuleProvider {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(AWSFirehoseReceiverModuleProvider.class);
    ...
    public void prepare() throws ServiceNotProvidedException {
        ...
        HTTPServerConfig.builder().host(moduleConfig.getHost()).port(moduleConfig.getPort()).contextPath(moduleConfig.getContextPath()).maxThreads(moduleConfig.getMaxThreads()).idleTimeOut(moduleConfig.getIdleTimeOut()).acceptQueueSize(moduleConfig.getAcceptQueueSize()).maxRequestHeaderSize(moduleConfig.getMaxRequestHeaderSize()).acceptProxyRequest(moduleConfig.isEnableTLS()).enableTLS(moduleConfig.isEnableTLS()).tlsKeyPath(moduleConfig.getTlsKeyPath()).tlsCertChainPath(moduleConfig.getTlsCertChainPath()).build();
    ...
    public void start() throws ServiceNotProvidedException, ModuleStartException {
        final OpenTelemetryMetricRequestProcessor processor = getManager().find(OtelMetricReceiverModule.NAME).provider().getService(OpenTelemetryMetricRequestProcessor.class);
        httpServer.addHandler(new FirehoseHTTPHandler(processor, moduleConfig.getFirehoseAccessKey()), Collections.singletonList(HttpMethod.POST));
    }
