...
public class K8sALSServiceMeshTCPAnalysis extends AbstractTCPAccessLogAnalyzer {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(K8sALSServiceMeshTCPAnalysis.class);
    ...
    @Override
    public Result analysis(final Result previousResult, final StreamAccessLogsMessage.Identifier identifier, final TCPAccessLogEntry entry, final Role role) {
        switch (role) {
        case PROXY: 
    ...
    protected Result analyzeSideCar(final Result previousResult, final TCPAccessLogEntry entry) {
        ...
        final Address downstreamRemoteAddress = properties.hasDownstreamDirectRemoteAddress() ? properties.getDownstreamDirectRemoteAddress() : properties.getDownstreamRemoteAddress();
    ...
    protected Result analyzeProxy(final Result previousResult, final TCPAccessLogEntry entry) {
        ...
        if (!previousResult.hasUpstreamMetrics()) {
            ...
            newAdapter(entry, ingress, targetService).adaptToUpstreamMetrics().setTlsMode(NON_TLS);
