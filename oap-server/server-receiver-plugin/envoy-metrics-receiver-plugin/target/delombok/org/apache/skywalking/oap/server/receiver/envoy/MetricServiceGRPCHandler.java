...
public class MetricServiceGRPCHandler extends MetricsServiceGrpc.MetricsServiceImplBase {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MetricServiceGRPCHandler.class);
    ...
    public MetricServiceGRPCHandler(final ModuleManager moduleManager, final EnvoyMetricReceiverConfig config) throws ModuleStartException {
        ...
        converters = config.rules().stream().map(rule -> new PrometheusMetricConverter(rule, meterSystem)).collect(Collectors.toList());
    }
                    ...
                    if (service != null && StringUtil.isNotEmpty(service.getServiceName()) && StringUtil.isNotEmpty(service.getServiceInstanceName())) {
                        ...
                        for (final Metrics.MetricFamily metricFamily : list) {
                            ...
                            try (HistogramMetrics.Timer ignored = histogram.createTimer()) {
                                final ProtoMetricFamily2MetricsAdapter adapter = new ProtoMetricFamily2MetricsAdapter(metricFamily, config.getClusterManagerMetricsAdapter());
                                adapter.adapt().forEach(it -> {
                                    ...
                                    List<Metric> metricList = groupingMetrics.computeIfAbsent(it.getName(), name -> new ArrayList<>());
                                    metricList.add(it);
                                });
                            }
                        }
                        groupingMetrics.forEach((name, metrics) -> converters.forEach(converter -> converter.toMeter(metrics.stream())));
                    }
