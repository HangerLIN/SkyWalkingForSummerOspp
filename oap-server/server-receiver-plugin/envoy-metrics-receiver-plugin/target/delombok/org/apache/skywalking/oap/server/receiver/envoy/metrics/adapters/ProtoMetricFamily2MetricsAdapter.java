...
public class ProtoMetricFamily2MetricsAdapter {
    ...
    public Stream<Metric> adapt() {
        ...
        case COUNTER: 
            return metricFamily.getMetricList().stream().map(it -> Counter.builder().name(adaptMetricsName(it)).value(it.getCounter().getValue()).timestamp(adaptTimestamp(it)).labels(adaptLabels(it)).build());
        case GAUGE: 
            return metricFamily.getMetricList().stream().map(it -> Gauge.builder().name(adaptMetricsName(it)).value(adaptValue(it)).timestamp(adaptTimestamp(it)).labels(adaptLabels(it)).build());
        case HISTOGRAM: 
            return metricFamily.getMetricList().stream().map(it -> Histogram.builder().name(adaptMetricsName(it)).labels(adaptLabels(it)).sampleCount(it.getHistogram().getSampleCount()).sampleSum(it.getHistogram().getSampleSum()).buckets(buildBuckets(it.getHistogram().getBucketList())).build());
        default: 
            return Stream.of();
        }
    }
    ...
    public Map<String, String> adaptLabels(final Metrics.Metric metric) {
        Map<String, String> labels = metric.getLabelList().stream().collect(toMap(Metrics.LabelPair::getName, Metrics.LabelPair::getValue));
        if (metricFamily.getName().startsWith("cluster.")) {
    ...
    @lombok.Generated
    public ProtoMetricFamily2MetricsAdapter(final Metrics.MetricFamily metricFamily, final ClusterManagerMetricsAdapter clusterManagerMetricsAdapter) {
