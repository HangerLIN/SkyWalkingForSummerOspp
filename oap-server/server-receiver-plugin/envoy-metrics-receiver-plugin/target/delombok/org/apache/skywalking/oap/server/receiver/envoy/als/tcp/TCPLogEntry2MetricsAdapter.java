...
public class TCPLogEntry2MetricsAdapter {
    ...
    public TCPServiceMeshMetric.Builder adaptToDownstreamMetrics() {
        ...
        return adaptCommonPart().setStartTime(startTime).setEndTime(startTime + duration).setDetectPoint(DetectPoint.server);
    }
    ...
    public TCPServiceMeshMetric.Builder adaptToUpstreamMetrics() {
        ...
        return adaptCommonPart().setStartTime(outboundStartTime).setEndTime(outboundEndTime).setDetectPoint(DetectPoint.client);
    }
    ...
    public TCPServiceMeshMetric.Builder adaptCommonPart() {
        ...
        final long internalResponseLatencyNanos = properties.getTimeToLastDownstreamTxByte().getNanos() - properties.getTimeToFirstUpstreamRxByte().getNanos();
        ...
        Optional.ofNullable(sourceService).map(ServiceMetaInfo::getServiceName).ifPresent(builder::setSourceServiceName);
        Optional.ofNullable(sourceService).map(ServiceMetaInfo::getServiceInstanceName).ifPresent(builder::setSourceServiceInstance);
        Optional.ofNullable(targetService).map(ServiceMetaInfo::getServiceName).ifPresent(builder::setDestServiceName);
        Optional.ofNullable(targetService).map(ServiceMetaInfo::getServiceInstanceName).ifPresent(builder::setDestServiceInstance);
        Optional.ofNullable(sourceService).map(ServiceMetaInfo::getTags).ifPresent(tags -> {
            tags.forEach(p -> {
                builder.addSourceInstanceProperties(KeyStringValuePair.newBuilder().setKey(p.getKey()).setValue(p.getValue()));
            });
        });
        Optional.ofNullable(targetService).map(ServiceMetaInfo::getTags).ifPresent(tags -> {
            tags.forEach(p -> {
                builder.addDestInstanceProperties(KeyStringValuePair.newBuilder().setKey(p.getKey()).setValue(p.getValue()));
            });
        });
        return builder;
    }
    ...
    @lombok.Generated
    public TCPLogEntry2MetricsAdapter(final TCPAccessLogEntry entry, final ServiceMetaInfo sourceService, final ServiceMetaInfo targetService) {
        this.entry = entry;
