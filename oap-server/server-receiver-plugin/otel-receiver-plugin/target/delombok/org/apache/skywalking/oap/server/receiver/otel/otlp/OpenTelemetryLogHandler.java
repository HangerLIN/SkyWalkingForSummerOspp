...
public class OpenTelemetryLogHandler extends LogsServiceGrpc.LogsServiceImplBase implements Handler {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OpenTelemetryLogHandler.class);
    ...
    public void active() throws ModuleStartException {
        GRPCHandlerRegister grpcHandlerRegister = manager.find(SharingServerModule.NAME).provider().getService(GRPCHandlerRegister.class);
    ...
    public void export(ExportLogsServiceRequest request, StreamObserver<ExportLogsServiceResponse> responseObserver) {
        request.getResourceLogsList().forEach(resourceLogs -> {
            ...
            final var attributes = resource.getAttributesList().stream().map(it -> Map.entry(it.getKey(), buildTagValue(it))).collect(toMap(Map.Entry::getKey, Map.Entry::getValue));
            ...
            resourceLogs.getScopeLogsList().stream().flatMap(it -> it.getLogRecordsList().stream()).forEach(logRecord -> doAnalysisQuietly(service, layer, serviceInstance, logRecord));
    ...
    private void doAnalysisQuietly(String service, String layer, String serviceInstance, LogRecord logRecord) {
        try {
            logAnalyzerService().doAnalysis(LogData.newBuilder().setService(service).setServiceInstance(serviceInstance).setTimestamp(logRecord.getTimeUnixNano() / 1000000).setTags(buildTags(logRecord)).setBody(buildBody(logRecord)).setLayer(layer), null);
        } catch (Exception e) {
    ...
    private LogTags buildTags(LogRecord logRecord) {
        return LogTags.newBuilder().addAllData(logRecord.getAttributesList().stream().collect(toMap(KeyValue::getKey, this::buildTagValue)).entrySet().stream().map(it -> KeyStringValuePair.newBuilder().setKey(it.getKey()).setValue(it.getValue()).build()).collect(Collectors.toList())).build();
    }
    ...
    private ILogAnalyzerService logAnalyzerService() {
        if (logAnalyzerService == null) {
            logAnalyzerService = manager.find(LogAnalyzerModule.NAME).provider().getService(ILogAnalyzerService.class);
        }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public OpenTelemetryLogHandler(final ModuleManager manager) {
