...
public class KafkaHandler {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(KafkaHandler.class);
    ...
    public KafkaHandler(final ZipkinReceiverConfig config, SpanForward forward, ModuleManager manager) {
        ...
        executor = new ThreadPoolExecutor(threadPoolSize, threadPoolSize, 60, TimeUnit.SECONDS, new ArrayBlockingQueue<>(threadPoolQueueSize), new CustomThreadFactory("Zipkin-Kafka-Consumer"), new ThreadPoolExecutor.CallerRunsPolicy());
        topics = Arrays.asList(config.getKafkaTopic().split(","));
        MetricsCreator metricsCreator = manager.find(TelemetryModule.NAME).provider().getService(MetricsCreator.class);
        histogram = metricsCreator.createHistogramMetric("trace_in_latency", "The process latency of trace data", new MetricsTag.Keys("protocol"), new MetricsTag.Values("zipkin-kafka"));
        msgDroppedIncr = metricsCreator.createCounter("trace_dropped_count", "The dropped number of traces", new MetricsTag.Keys("protocol"), new MetricsTag.Values("zipkin-kafka"));
        errorCounter = metricsCreator.createCounter("trace_analysis_error_count", "The error number of trace analysis", new MetricsTag.Keys("protocol"), new MetricsTag.Values("zipkin-kafka"));
    }
    ...
    private void runTask(final KafkaConsumer<byte[], byte[]> consumer) {
        ...
        while (true) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Consume zipkin trace records from kafka, records count:[{}].", consumerRecords.count());
                }
