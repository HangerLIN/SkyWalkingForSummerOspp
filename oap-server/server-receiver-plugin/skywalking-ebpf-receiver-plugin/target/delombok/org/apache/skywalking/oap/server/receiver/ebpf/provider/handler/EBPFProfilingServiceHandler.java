...
public class EBPFProfilingServiceHandler extends EBPFProfilingServiceGrpc.EBPFProfilingServiceImplBase implements GRPCHandler {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(EBPFProfilingServiceHandler.class);
    ...
    public void queryTasks(EBPFProfilingTaskQuery request, StreamObserver<Commands> responseObserver) {
        ...
        try {
            ...
            tasks.stream().collect(Collectors.toMap(EBPFProfilingTaskRecord::getLogicalId, Function.identity(), EBPFProfilingTaskRecord::combine)).values().stream().flatMap(t -> this.buildProfilingCommands(t, processes).stream()).map(EBPFProfilingTaskCommand::serialize).forEach(builder::addCommands);
    ...
    private List<EBPFProfilingTaskCommand> buildProfilingCommands(EBPFProfilingTaskRecord task, List<Process> processes) {
        ...
        for (Process process : processes) {
            ...
            if (!Objects.equals(process.getServiceId(), task.getServiceId()) || !ProfilingSupportStatus.SUPPORT_EBPF_PROFILING.name().equals(process.getProfilingSupportStatus())) {
                continue;
            }
            ...
            if (CollectionUtils.isEmpty(processLabels) || process.getLabels().containsAll(processLabels)) {
    ...
    private void processOnCPUProfiling(EBPFProfilingData data, EBPFOnCPUProfiling onCPU) throws IOException {
        ...
        data.setDataBinary(EBPFOnCPUProfiling.newBuilder().addAllStacks(order._2).setDumpCount(onCPU.getDumpCount()).build().toByteArray());
    }
    ...
    private void processOffCPUProfiling(EBPFProfilingData data, EBPFOffCPUProfiling offCPUProfiling) throws IOException {
        ...
        data.setDataBinary(EBPFOffCPUProfiling.newBuilder().addAllStacks(order._2).setSwitchCount(offCPUProfiling.getSwitchCount()).setDuration(offCPUProfiling.getDuration()).build().toByteArray());
    }

    private Tuple2<String, List<EBPFProfilingStackMetadata>> orderMetadataAndSetToData(List<EBPFProfilingStackMetadata> original, List<EBPFProfilingStackType> order) {
