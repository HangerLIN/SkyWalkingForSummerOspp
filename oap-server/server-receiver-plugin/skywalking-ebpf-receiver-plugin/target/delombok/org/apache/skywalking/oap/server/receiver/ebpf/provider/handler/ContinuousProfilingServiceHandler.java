...
public class ContinuousProfilingServiceHandler extends ContinuousProfilingServiceGrpc.ContinuousProfilingServiceImplBase implements GRPCHandler {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ContinuousProfilingServiceHandler.class);
    ...
    public ContinuousProfilingServiceHandler(ModuleManager moduleManager, EBPFReceiverModuleConfig config) {
        ...
        this.policyCache = CacheBuilder.newBuilder().expireAfterWrite(config.getContinuousPolicyCacheTimeout(), TimeUnit.SECONDS).build();
    }

    @Override
    public void queryPolicies(ContinuousProfilingPolicyQuery request, StreamObserver<Commands> responseObserver) {
        final Map<String, String> policiesQuery = request.getPoliciesList().stream().collect(Collectors.toMap(s -> IDManager.ServiceID.buildId(s.getServiceName(), true), ContinuousProfilingServicePolicyQuery::getUuid, (s1, s2) -> s1));
    ...
    private static class PolicyWrapper {
        ...
        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public PolicyWrapper(final ContinuousProfilingPolicy policy) {
            this.policy = policy;
        }
    }
}
