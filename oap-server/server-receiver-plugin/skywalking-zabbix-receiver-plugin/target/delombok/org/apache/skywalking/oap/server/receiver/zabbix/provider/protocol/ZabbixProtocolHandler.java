...
public class ZabbixProtocolHandler extends SimpleChannelInboundHandler<ZabbixRequest> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ZabbixProtocolHandler.class);
    ...
    public void channelRead0(ChannelHandlerContext ctx, ZabbixRequest msg) {
        if (msg.getType() == ZabbixProtocolType.ACTIVE_CHECKS) {
            ...
            response.setActiveChecks(Optional.of(metrics.getAllMonitorMetricNames(hostName)).map(s -> s.stream().map(key -> ZabbixResponse.ActiveChecks.builder().delay(60).lastlogsize(0).key(key).mtime(0).build()).collect(Collectors.toList())).orElse(Collections.emptyList()));
