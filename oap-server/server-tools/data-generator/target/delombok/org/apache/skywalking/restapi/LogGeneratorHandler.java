...
public class LogGeneratorHandler {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LogGeneratorHandler.class);
    ...
    public HttpResponse generateMockLogs(@Default("0") @Param("size") int size, @Default("0") @Param("qps") int qps, @RequestObject LogRequest request) {
        if (size > 0 && qps > 0) {
        ...
        future.addListener(f -> {
            if (f.isDone()) {
                ...
                log.info("Generate mock logs finished: {}, requestId: {}", f.isSuccess(), requestId);
            }
            if (f.cause() != null) {
    ...
    public HttpResponse cancelRequest(@Param("requestId") String requestId) {
        ...
        if (future == null) {
            return HttpResponse.of(HttpStatus.NOT_FOUND, MediaType.PLAIN_TEXT_UTF_8, "No such request: %s", requestId);
        }
