...
public class ProfileSnapshotDumper {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ProfileSnapshotDumper.class);
        ...
        try (OutputStream outputStream = new BufferedOutputStream(new FileOutputStream(snapshotFile))) {
            for (int i = 0; i < rangeCount; i++) {
                ...
                for (ProfileThreadSnapshotRecord record : records) {
                    ...
                    ThreadSnapshot.newBuilder().setStack(ThreadStack.parseFrom(record.getStackBinary())).setSequence(record.getSequence()).setTime(record.getDumpTime()).build().writeDelimitedTo(outputStream);
                }
