...
public class HTTPServer implements Server {
    ...
    public void initialize() {
        ...
        sb = com.linecorp.armeria.server.Server.builder().serviceUnder(contextPath + "/docs", DocService.builder().build()).service("/internal/l7check", HealthCheckService.of()).workerGroup(config.getMaxThreads()).http1MaxHeaderSize(config.getMaxRequestHeaderSize()).idleTimeout(Duration.ofMillis(config.getIdleTimeOut())).decorator(Route.ofCatchAll(), (delegate, ctx, req) -> {
        ...
        if (config.isEnableTLS()) {
            sb.https(new InetSocketAddress(config.getHost(), config.getPort()));
            try (
                InputStream cert = new FileInputStream(config.getTlsCertChainPath());
            ...
            sb.http(new InetSocketAddress(config.getHost(), config.getPort()));
        }
    ...
    public void addHandler(Object handler, List<HttpMethod> httpMethods) {
        ...
        log.info("Bind handler {} into http server {}:{}", handler.getClass().getSimpleName(), config.getHost(), config.getPort());
        sb.annotatedService().pathPrefix(config.getContextPath()).build(handler);
