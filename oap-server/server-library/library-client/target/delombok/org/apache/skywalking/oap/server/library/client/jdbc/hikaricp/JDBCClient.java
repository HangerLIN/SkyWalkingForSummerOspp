...
public class JDBCClient implements Client, HealthCheckable {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JDBCClient.class);
    ...
    public boolean indexExists(final String table, final String index) throws SQLException {
        try (
            var connection = getConnection();
            var resultSet = connection.getMetaData().getIndexInfo(connection.getCatalog(), null, table, false, false)) {
    ...
    public boolean tableExists(final String table) throws SQLException {
        try (
            ...
            var result = conn.getMetaData().getTables(conn.getCatalog(), null, table, null)) {
            return result.next();
        }
    }
    ...
    public Set<String> getTableColumns(final String table) throws SQLException {
        try (
            var conn = getConnection();
            var result = conn.getMetaData().getColumns(conn.getCatalog(), null, table, null)) {
