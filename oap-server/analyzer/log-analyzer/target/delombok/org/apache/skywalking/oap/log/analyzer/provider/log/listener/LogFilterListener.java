...
public class LogFilterListener implements LogAnalysisListener {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LogFilterListener.class);
    ...
    @Override
    public LogAnalysisListener parse(final LogData.Builder logData, final Message extraLog) {
        dsls.forEach(dsl -> dsl.bind(new Binding().log(logData.build()).extraLog(extraLog)));
        return this;
    }
    ...
    public static class Factory implements LogAnalysisListenerFactory {
        ...
        public Factory(final ModuleManager moduleManager, final LogAnalyzerModuleConfig config) throws Exception {
            dsls = new HashMap<>();
            final List<LALConfig> configList = LALConfigs.load(config.getLalPath(), config.lalFiles()).stream().flatMap(it -> it.getRules().stream()).collect(Collectors.toList());
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public LogFilterListener(final Collection<DSL> dsls) {
        this.dsls = dsls;
    }
}
