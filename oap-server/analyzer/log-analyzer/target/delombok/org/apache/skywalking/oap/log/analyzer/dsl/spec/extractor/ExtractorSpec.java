...
public class ExtractorSpec extends AbstractSpec {
    ...
    public ExtractorSpec(final ModuleManager moduleManager, final LogAnalyzerModuleConfig moduleConfig) throws ModuleStartException {
        super(moduleManager, moduleConfig);
        final MeterSystem meterSystem = moduleManager.find(CoreModule.NAME).provider().getService(MeterSystem.class);
        metricConverts = moduleConfig.malConfigs().stream().map(it -> new MetricConvert(it, meterSystem)).collect(Collectors.toList());
    ...
    public void tag(final Map<String, ?> kv) {
        ...
        logData.setTags(logData.getTags().toBuilder().addAllData(kv.entrySet().stream().filter(it -> isNotBlank(it.getKey())).filter(it -> nonNull(it.getValue()) && isNotBlank(Objects.toString(it.getValue()))).map(it -> {
            final Object val = it.getValue();
            String valStr = Objects.toString(val);
            if (Collection.class.isAssignableFrom(val.getClass())) {
                valStr = Joiner.on(",").skipNulls().join((Collection<?>) val);
            }
            return KeyStringValuePair.newBuilder().setKey(it.getKey()).setValue(valStr).build();
    ...
    public void metrics(@DelegatesTo(SampleBuilder.class) final Closure<?> cl) {
        ...
        if (possibleMetricsContainer.isPresent()) {
            ...
            metricConverts.forEach(it -> it.toMeter(ImmutableMap.<String, SampleFamily>builder().put(sample.getName(), sampleFamily).build()));
        }
    }
    ...
    public void slowSql(@DelegatesTo(SlowSqlSpec.class) final Closure<?> cl) {
        ...
        if (log.getLayer() == null || log.getService() == null || log.getTimestamp() < 1) {
        ...
        cl.call();
        if (builder.getId() == null || builder.getLatency() < 1 || builder.getStatement() == null) {
            LOGGER.warn("SlowSql extracts failed, maybe something is not configured.");
            return;
        }
    ...
    public static class SampleBuilder {
        ...
        public Sample.SampleBuilder labels(final Map<String, ?> labels) {
            final Map<String, String> filtered = labels.entrySet().stream().filter(it -> isNotBlank(it.getKey()) && nonNull(it.getValue())).collect(Collectors.toMap(Map.Entry::getKey, it -> Objects.toString(it.getValue())));
            return sampleBuilder.labels(ImmutableMap.copyOf(filtered));
        }

        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public org.apache.skywalking.oap.meter.analyzer.dsl.Sample.SampleBuilder name(final java.lang.String name) {
            return this.sampleBuilder.name(name);
        }

        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public org.apache.skywalking.oap.meter.analyzer.dsl.Sample.SampleBuilder labels(final com.google.common.collect.ImmutableMap<java.lang.String, java.lang.String> labels) {
            return this.sampleBuilder.labels(labels);
        }

        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public org.apache.skywalking.oap.meter.analyzer.dsl.Sample.SampleBuilder value(final double value) {
            return this.sampleBuilder.value(value);
        }

        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public org.apache.skywalking.oap.meter.analyzer.dsl.Sample.SampleBuilder timestamp(final long timestamp) {
            return this.sampleBuilder.timestamp(timestamp);
        }

        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public org.apache.skywalking.oap.meter.analyzer.dsl.Sample build() {
