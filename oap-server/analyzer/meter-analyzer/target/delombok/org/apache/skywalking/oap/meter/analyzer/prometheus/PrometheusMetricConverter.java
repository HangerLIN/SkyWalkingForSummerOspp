...
public class PrometheusMetricConverter {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PrometheusMetricConverter.class);
    private final Pattern metricsNameEscapePattern;
    private final LoadingCache<String, String> escapedMetricsNameCache = CacheBuilder.newBuilder().maximumSize(1000).build(new CacheLoader<String, String>() {
        @Override
    ...
    public ImmutableMap<String, SampleFamily> convertPromMetricToSampleFamily(Stream<Metric> metricStream) {
        return metricStream.peek(metric -> log.debug("Prom metric to be convert to SampleFamily: {}", metric)).flatMap(this::convertMetric).filter(t -> t != NIL && t._2.samples.length > 0).peek(t -> log.debug("SampleFamily: {}", t)).collect(toImmutableMap(Tuple2::_1, Tuple2::_2, (a, b) -> {
            log.debug("merge {} {}", a, b);
