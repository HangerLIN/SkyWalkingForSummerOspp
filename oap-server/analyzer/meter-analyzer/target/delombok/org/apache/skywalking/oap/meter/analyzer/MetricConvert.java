...
public class MetricConvert {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MetricConvert.class);
    ...
    public MetricConvert(MetricRuleConfig rule, MeterSystem service) {
        ...
        this.analyzers = rule.getMetricsRules().stream().map(r -> buildAnalyzer(formatMetricName(rule, r.getName()), rule.getFilter(), formatExp(rule.getExpPrefix(), rule.getExpSuffix(), r.getExp()), service)).collect(toList());
    }

    Analyzer buildAnalyzer(final String metricsName, final String filter, final String exp, final MeterSystem service) {
