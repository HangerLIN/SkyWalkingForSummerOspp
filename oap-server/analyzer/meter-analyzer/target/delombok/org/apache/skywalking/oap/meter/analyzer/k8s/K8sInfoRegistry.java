...
public class K8sInfoRegistry {
    ...
    /* Pod */ /* Service */private final LoadingCache<ObjectID, ObjectID> podServiceMap;
    /* podIP */ /* Pod */private final LoadingCache<String, ObjectID> ipPodMap;
    /* serviceIP */ /* Service */private final LoadingCache<String, ObjectID> ipServiceMap;

    private K8sInfoRegistry() {
        ipPodMap = CacheBuilder.newBuilder().expireAfterWrite(Duration.ofMinutes(3)).build(CacheLoader.from(ip -> KubernetesPods.INSTANCE.findByIP(ip).map(it -> ObjectID.builder().name(it.getMetadata().getName()).namespace(it.getMetadata().getNamespace()).build()).orElse(ObjectID.EMPTY)));
        ...
        podServiceMap = CacheBuilder.newBuilder().expireAfterWrite(Duration.ofMinutes(3)).build(CacheLoader.from(podObjectID -> {
            final Optional<Pod> pod = KubernetesPods.INSTANCE.findByObjectID(ObjectID.builder().name(podObjectID.name()).namespace(podObjectID.namespace()).build());
            if (!pod.isPresent() || pod.get().getMetadata() == null || pod.get().getMetadata().getLabels() == null) {
                return ObjectID.EMPTY;
            }
            final Optional<Service> service = KubernetesServices.INSTANCE.list().stream().filter(it -> it.getMetadata() != null).filter(it -> Objects.equals(it.getMetadata().getNamespace(), pod.get().getMetadata().getNamespace())).filter(it -> it.getSpec() != null).filter(it -> requireNonNull(it.getSpec()).getSelector() != null).filter(it -> {
    ...
    public String findServiceName(String namespace, String podName) {
        try {
            return this.podServiceMap.get(ObjectID.builder().name(podName).namespace(namespace).build()).toString();
