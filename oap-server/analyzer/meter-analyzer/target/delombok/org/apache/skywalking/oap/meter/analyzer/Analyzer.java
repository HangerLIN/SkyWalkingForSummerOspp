...
public class Analyzer {
    ...
    public static Analyzer build(final String metricName, final String filterExpression, final String expression, final MeterSystem meterSystem) {
    ...
    public void analyse(final ImmutableMap<String, SampleFamily> sampleFamilies) {
        Map<String, SampleFamily> input = samples.stream().map(s -> Tuple.of(s, sampleFamilies.get(s))).filter(t -> t._2 != null).collect(toImmutableMap(t -> t._1, t -> t._2));
        if (input.size() < 1) {
        ...
        meterSamples.forEach((meterEntity, ss) -> {
            ...
            case histogramPercentile: 
                Stream.of(ss).map(s -> Tuple.of(composeGroup(s.getLabels(), k -> !Objects.equals("le", k)), s)).collect(groupingBy(Tuple2::_1, mapping(Tuple2::_2, toList()))).forEach((group, subSs) -> {
                    if (subSs.size() < 1) {
                        return;
                    }
    ...
    private String composeGroup(ImmutableMap<String, String> labels, Predicate<String> filter) {
        return labels.keySet().stream().filter(filter).sorted().map(labels::get).collect(Collectors.joining("-"));
    }
    ...
    private enum MetricType {
        ...
        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        private MetricType(final String literal) {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    private Analyzer(final String metricName, final FilterExpression filterExpression, final Expression expression, final MeterSystem meterSystem, final ExpressionParsingContext ctx) {
