...
public class SampleFamily {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SampleFamily.class);
    ...
    public SampleFamily avg(List<String> by) {
        ...
        if (by == null) {
            double result = Arrays.stream(samples).mapToDouble(Sample::getValue).average().orElse(0.0);
            return SampleFamily.build(this.context, InternalOps.newSample(samples[0].name, ImmutableMap.of(), samples[0].timestamp, result));
        }
        return SampleFamily.build(this.context, Arrays.stream(samples).collect(groupingBy(it -> InternalOps.getLabels(by, it), mapping(identity(), toList()))).entrySet().stream().map(entry -> InternalOps.newSample(entry.getValue().get(0).getName(), entry.getKey(), entry.getValue().get(0).getTimestamp(), entry.getValue().stream().mapToDouble(Sample::getValue).average().orElse(0.0))).toArray(Sample[]::new));
    }
    ...
    protected SampleFamily aggregate(List<String> by, DoubleBinaryOperator aggregator) {
        ...
        if (by == null) {
            double result = Arrays.stream(samples).mapToDouble(s -> s.value).reduce(aggregator).orElse(0.0);
            return SampleFamily.build(this.context, InternalOps.newSample(samples[0].name, ImmutableMap.of(), samples[0].timestamp, result));
        }
        return SampleFamily.build(this.context, Arrays.stream(samples).collect(groupingBy(it -> InternalOps.getLabels(by, it), mapping(identity(), toList()))).entrySet().stream().map(entry -> InternalOps.newSample(entry.getValue().get(0).getName(), entry.getKey(), entry.getValue().get(0).getTimestamp(), entry.getValue().stream().mapToDouble(Sample::getValue).reduce(aggregator).orElse(0.0))).toArray(Sample[]::new));
    }
    ...
    public SampleFamily increase(String range) {
        ...
        return SampleFamily.build(this.context, Arrays.stream(samples).map(sample -> sample.increase(range, context.metricName, (lowerBoundValue, unused) -> sample.value - lowerBoundValue)).toArray(Sample[]::new));
    }
    ...
    public SampleFamily rate(String range) {
        ...
        return SampleFamily.build(this.context, Arrays.stream(samples).map(sample -> sample.increase(range, context.metricName, (lowerBoundValue, lowerBoundTime) -> {
    ...
    public SampleFamily irate() {
        ...
        return SampleFamily.build(this.context, Arrays.stream(samples).map(sample -> sample.increase(context.metricName, (lowerBoundValue, lowerBoundTime) -> {
            final long timeDiff = (sample.timestamp - lowerBoundTime) / 1000;
            return timeDiff < 1L ? 0.0 : (sample.value - lowerBoundValue) / timeDiff;
        })).toArray(Sample[]::new));
    }
    ...
    public SampleFamily tag(Closure<?> cl) {
        ...
        return SampleFamily.build(this.context, Arrays.stream(samples).map(sample -> {
            ...
            Object r = c.call(arg);
            return sample.toBuilder().labels(ImmutableMap.copyOf(Optional.ofNullable((r instanceof Map) ? (Map<String, String>) r : null).orElse(arg))).build();
        }).toArray(Sample[]::new));
    }
    ...
    public SampleFamily filter(Closure<Boolean> filter) {
        ...
        final Sample[] filtered = Arrays.stream(samples).filter(it -> filter.call(it.labels)).toArray(Sample[]::new);
    ...
    public SampleFamily histogram(String le, TimeUnit unit) {
        ...
        return SampleFamily.build(this.context, Stream.concat(Arrays.stream(samples).filter(s -> !s.labels.containsKey(le)), Arrays.stream(samples).filter(s -> s.labels.containsKey(le)).sorted(Comparator.comparingDouble(s -> Double.parseDouble(s.labels.get(le)))).map(s -> {
            double r = s.value;
            ImmutableMap<String, String> ll = ImmutableMap.<String, String>builder().putAll(Maps.filterKeys(s.labels, key -> !Objects.equals(key, le))).put("le", String.valueOf((long) ((Double.parseDouble(s.labels.get(le))) * scale))).build();
    ...
    public SampleFamily instance(List<String> serviceKeys, String serviceDelimiter, List<String> instanceKeys, String instanceDelimiter, Layer layer, Closure<Map<String, String>> propertiesExtractor) {
        ...
        return createMeterSamples(new InstanceEntityDescription(serviceKeys, instanceKeys, layer, serviceDelimiter, instanceDelimiter, propertiesExtractor));
    }
    ...
    private SampleFamily createMeterSamples(EntityDescription entityDescription) {
        ...
        Arrays.stream(samples).collect(groupingBy(it -> InternalOps.getLabels(entityDescription.getLabelKeys(), it), mapping(identity(), toList()))).forEach((labels, samples) -> {
    ...
    private SampleFamily match(String[] labels, Function2<String, String, Boolean> op) {
        ...
        Sample[] ss = Arrays.stream(samples).filter(sample -> ll.entrySet().stream().allMatch(entry -> op.apply(sample.labels.getOrDefault(entry.getKey(), ""), entry.getValue()))).toArray(Sample[]::new);
    ...
    private SampleFamily valueMatch(CompType compType, double compValue, Function3<CompType, Double, Double, Boolean> op) {
        Sample[] ss = Arrays.stream(samples).filter(sample -> op.apply(compType, sample.value, compValue)).toArray(Sample[]::new);
        return ss.length > 0 ? SampleFamily.build(this.context, ss) : EMPTY;
    }
    ...
    private SampleFamily newValue(SampleFamily another, Function2<Double, Double, Double> transform) {
        Sample[] ss = Arrays.stream(samples).flatMap(cs -> io.vavr.collection.Stream.of(another.samples).find(as -> cs.labels.equals(as.labels)).map(as -> cs.toBuilder().value(transform.apply(cs.value, as.value))).map(Sample.SampleBuilder::build).toJavaStream()).toArray(Sample[]::new);
        return ss.length > 0 ? SampleFamily.build(this.context, ss) : EMPTY;
    }
    ...
    public static class RunningContext {
        ...
        static RunningContext instance() {
            return RunningContext.builder().defaultHistogramBucketUnit(TimeUnit.SECONDS).build();
        }
        ...
        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        RunningContext(final String metricName, final Map<MeterEntity, Sample[]> meterSamples, final TimeUnit defaultHistogramBucketUnit) {
        ...
        public static class RunningContextBuilder {
            ...
            @java.lang.SuppressWarnings("all")
            @lombok.Generated
            public SampleFamily.RunningContext.RunningContextBuilder metricName(final String metricName) {
                this.metricName = metricName;
                return this;
            }
            ...
            @java.lang.SuppressWarnings("all")
            @lombok.Generated
            public SampleFamily.RunningContext.RunningContextBuilder meterSamples(final Map<MeterEntity, Sample[]> meterSamples) {
                this.meterSamples$value = meterSamples;
                meterSamples$set = true;
                return this;
            }
            ...
            @java.lang.SuppressWarnings("all")
            @lombok.Generated
            public SampleFamily.RunningContext.RunningContextBuilder defaultHistogramBucketUnit(final TimeUnit defaultHistogramBucketUnit) {
                this.defaultHistogramBucketUnit = defaultHistogramBucketUnit;
                return this;
            }
            ...
            @java.lang.SuppressWarnings("all")
            @lombok.Generated
            public SampleFamily.RunningContext build() {
                Map<MeterEntity, Sample[]> meterSamples$value = this.meterSamples$value;
                if (!this.meterSamples$set) meterSamples$value = RunningContext.$default$meterSamples();
                return new SampleFamily.RunningContext(this.metricName, meterSamples$value, this.defaultHistogramBucketUnit);
            }
        ...
        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public static SampleFamily.RunningContext.RunningContextBuilder builder() {
            return new SampleFamily.RunningContext.RunningContextBuilder();
        }
        ...
        @lombok.Generated
        public java.lang.String toString() {
            return "SampleFamily.RunningContext(metricName=" + this.getMetricName() + ", meterSamples=" + this.getMeterSamples() + ", defaultHistogramBucketUnit=" + this.getDefaultHistogramBucketUnit() + ")";
        }
        ...
        public boolean equals(final java.lang.Object o) {
            ...
            if (!(o instanceof SampleFamily.RunningContext)) return false;
            final SampleFamily.RunningContext other = (SampleFamily.RunningContext) o;
            if (!other.canEqual((java.lang.Object) this)) return false;
            final java.lang.Object this$meterSamples = this.getMeterSamples();
            final java.lang.Object other$meterSamples = other.getMeterSamples();
            if (this$meterSamples == null ? other$meterSamples != null : !this$meterSamples.equals(other$meterSamples)) return false;
            final java.lang.Object this$defaultHistogramBucketUnit = this.getDefaultHistogramBucketUnit();
            final java.lang.Object other$defaultHistogramBucketUnit = other.getDefaultHistogramBucketUnit();
            if (this$defaultHistogramBucketUnit == null ? other$defaultHistogramBucketUnit != null : !this$defaultHistogramBucketUnit.equals(other$defaultHistogramBucketUnit)) return false;
            return true;
        }
        ...
        protected boolean canEqual(final java.lang.Object other) {
            return other instanceof SampleFamily.RunningContext;
        }
        ...
        public int hashCode() {
            final int PRIME = 59;
            int result = 1;
            final java.lang.Object $meterSamples = this.getMeterSamples();
            result = result * PRIME + ($meterSamples == null ? 43 : $meterSamples.hashCode());
            final java.lang.Object $defaultHistogramBucketUnit = this.getDefaultHistogramBucketUnit();
            result = result * PRIME + ($defaultHistogramBucketUnit == null ? 43 : $defaultHistogramBucketUnit.hashCode());
            return result;
        }
        ...
        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public void setMeterSamples(final Map<MeterEntity, Sample[]> meterSamples) {
            this.meterSamples = meterSamples;
        }

        @java.lang.SuppressWarnings("all")
        @lombok.Generated
        public void setDefaultHistogramBucketUnit(final TimeUnit defaultHistogramBucketUnit) {
            this.defaultHistogramBucketUnit = defaultHistogramBucketUnit;
        }
    }
    ...
    private static class InternalOps {
        private static Sample[] left(List<Sample> samples, List<String> labelKeys) {
            return samples.stream().map(s -> {
                ImmutableMap<String, String> ll = ImmutableMap.<String, String>builder().putAll(Maps.filterKeys(s.labels, key -> !labelKeys.contains(key))).build();
                return s.toBuilder().labels(ll).build();
            }).toArray(Sample[]::new);
        }

        private static String dim(List<Sample> samples, List<String> labelKeys, String delimiter) {
            String name = labelKeys.stream().map(k -> samples.get(0).labels.getOrDefault(k, "")).filter(v -> !StringUtil.isEmpty(v)).collect(Collectors.joining(StringUtil.isEmpty(delimiter) ? Const.POINT : delimiter));
            return name;
        }
        ...
        private static MeterEntity buildMeterEntity(List<Sample> samples, EntityDescription entityDescription) {
            ...
            case SERVICE: 
                ...
                return MeterEntity.newService(InternalOps.dim(samples, serviceEntityDescription.getServiceKeys(), serviceEntityDescription.getDelimiter()), serviceEntityDescription.getLayer());
            case SERVICE_INSTANCE: 
                ...
                return MeterEntity.newServiceInstance(InternalOps.dim(samples, instanceEntityDescription.getServiceKeys(), instanceEntityDescription.getServiceDelimiter()), InternalOps.dim(samples, instanceEntityDescription.getInstanceKeys(), instanceEntityDescription.getInstanceDelimiter()), instanceEntityDescription.getLayer(), properties);
            case ENDPOINT: 
                EndpointEntityDescription endpointEntityDescription = (EndpointEntityDescription) entityDescription;
                return MeterEntity.newEndpoint(InternalOps.dim(samples, endpointEntityDescription.getServiceKeys(), endpointEntityDescription.getDelimiter()), InternalOps.dim(samples, endpointEntityDescription.getEndpointKeys(), endpointEntityDescription.getDelimiter()), endpointEntityDescription.getLayer());
            case PROCESS: 
                final ProcessEntityDescription processEntityDescription = (ProcessEntityDescription) entityDescription;
                return MeterEntity.newProcess(InternalOps.dim(samples, processEntityDescription.getServiceKeys(), processEntityDescription.getDelimiter()), InternalOps.dim(samples, processEntityDescription.getServiceInstanceKeys(), processEntityDescription.getDelimiter()), InternalOps.dim(samples, processEntityDescription.getProcessKeys(), processEntityDescription.getDelimiter()), InternalOps.dim(samples, List.of(processEntityDescription.getLayerKey()), processEntityDescription.getDelimiter()));
            case SERVICE_RELATION: 
                ...
                final String serviceRelationComponentValue = InternalOps.dim(samples, Collections.singletonList(serviceRelationEntityDescription.getComponentIdKey()), serviceRelationEntityDescription.getDelimiter());
                ...
                return MeterEntity.newServiceRelation(InternalOps.dim(samples, serviceRelationEntityDescription.getSourceServiceKeys(), serviceRelationEntityDescription.getDelimiter()), InternalOps.dim(samples, serviceRelationEntityDescription.getDestServiceKeys(), serviceRelationEntityDescription.getDelimiter()), serviceRelationEntityDescription.getDetectPoint(), serviceRelationEntityDescription.getLayer(), serviceRelationComponentId);
            case PROCESS_RELATION: 
                ...
                return MeterEntity.newProcessRelation(InternalOps.dim(samples, processRelationEntityDescription.getServiceKeys(), processRelationEntityDescription.getDelimiter()), InternalOps.dim(samples, processRelationEntityDescription.getInstanceKeys(), processRelationEntityDescription.getDelimiter()), InternalOps.dim(samples, Collections.singletonList(processRelationEntityDescription.getSourceProcessIdKey()), processRelationEntityDescription.getDelimiter()), InternalOps.dim(samples, Collections.singletonList(processRelationEntityDescription.getDestProcessIdKey()), processRelationEntityDescription.getDelimiter()), componentId, point);
            default: 
        ...
        private static ImmutableMap<String, String> getLabels(final List<String> labelKeys, final Sample sample) {
            return labelKeys.stream().collect(toImmutableMap(Function.identity(), labelKey -> sample.labels.getOrDefault(labelKey, "")));
        }
    }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    private SampleFamily(final Sample[] samples, final RunningContext context) {
    ...
    public boolean equals(final java.lang.Object o) {
        if (o == this) return true;
        if (!(o instanceof SampleFamily)) return false;
        final SampleFamily other = (SampleFamily) o;
        if (!other.canEqual((java.lang.Object) this)) return false;
        if (!java.util.Arrays.deepEquals(this.samples, other.samples)) return false;
        final java.lang.Object this$context = this.context;
        final java.lang.Object other$context = other.context;
    ...
    public int hashCode() {
        final int PRIME = 59;
        int result = 1;
        result = result * PRIME + java.util.Arrays.deepHashCode(this.samples);
        final java.lang.Object $context = this.context;
    ...
    @lombok.Generated
    public java.lang.String toString() {
        return "SampleFamily(samples=" + java.util.Arrays.deepToString(this.samples) + ", context=" + this.context + ")";
    }
}
