...
public class MeterProcessor {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MeterProcessor.class);
    /**
     * Process context.
     */
    ...
    public void process() {
        ...
        try {
            converts.forEach(convert -> convert.toMeter(meters.entrySet().stream().collect(toImmutableMap(Map.Entry::getKey, v -> SampleFamilyBuilder.newBuilder(v.getValue().stream().map(s -> s.build(service, serviceInstance, timestamp)).toArray(Sample[]::new)).defaultHistogramBucketUnit(TimeUnit.MILLISECONDS).build()))));
        } catch (Exception e) {
