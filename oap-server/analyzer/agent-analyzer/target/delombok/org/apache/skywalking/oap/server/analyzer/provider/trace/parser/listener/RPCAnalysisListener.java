...
public class RPCAnalysisListener extends CommonAnalysisListener implements EntryAnalysisListener, ExitAnalysisListener, LocalAnalysisListener {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RPCAnalysisListener.class);
    ...
    public void parseEntry(SpanObject span, SegmentObject segmentObject) {
        ...
        if (span.getRefsCount() > 0) {
            for (int i = 0; i < span.getRefsCount(); i++) {
                ...
                if (isMQ || config.getUninstrumentedGatewaysConfig().isAddressConfiguredAsGateway(networkAddressUsedAtPeer)) {
    ...
    public void parseExit(SpanObject span, SegmentObject segmentObject) {
        ...
        if (networkAddressAlias == null) {
             ...
             */
            final IDManager.ServiceID.ServiceIDDefinition serviceIDDefinition = IDManager.ServiceID.analysisId(networkAddressAlias.getRepresentServiceId());
            final IDManager.ServiceInstanceID.InstanceIDDefinition instanceIDDefinition = IDManager.ServiceInstanceID.analysisId(networkAddressAlias.getRepresentServiceInstanceId());
    ...
    public static class Factory implements AnalysisListenerFactory {
        ...
        public AnalysisListener create(ModuleManager moduleManager, AnalyzerModuleConfig config) {
            return new RPCAnalysisListener(sourceReceiver, config, networkAddressAliasCache, namingControl);
        }
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public RPCAnalysisListener(final SourceReceiver sourceReceiver, final AnalyzerModuleConfig config, final NetworkAddressAliasCache networkAddressAliasCache, final NamingControl namingControl) {
