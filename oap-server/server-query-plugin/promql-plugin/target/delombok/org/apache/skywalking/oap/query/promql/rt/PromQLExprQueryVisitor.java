...
public class PromQLExprQueryVisitor extends PromQLParserBaseVisitor<ParseResult> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PromQLExprQueryVisitor.class);
    ...
    public PromQLExprQueryVisitor(final MetricsQuery metricsQuery, final RecordsQuery recordsQuery, final Duration duration, final PromQLApiHandler.QueryType queryType) {
    ...
    public ParseResult visitMetricInstant(PromQLParser.MetricInstantContext ctx) {
        ...
        try {
            ...
            try {
                ...
                throw new IllegalExpressionException("Layer:[" + labelMap.get(LabelName.LAYER) + "] is missing or illegal.");
            }
    ...
    private void queryTopN(String metricName, Layer layer, Scope scope, Map<LabelName, String> labelMap, MetricsRangeResult matrixResult) throws IOException, IllegalExpressionException {
        ...
        for (SelectedRecord selectedRecord : selectedRecords) {
            ...
            MetricInfo metricInfo = buildMetricInfo(metricName, layer, scope, labelMap, Optional.empty(), Optional.ofNullable(selectedRecord.getName()), Optional.empty());
    ...
    private void queryRecords(String metricName, Layer layer, Scope scope, Map<LabelName, String> labelMap, MetricsRangeResult matrixResult) throws IOException, IllegalExpressionException {
        ...
        for (Record record : records) {
            MetricRangeData metricData = new MetricRangeData();
            MetricInfo metricInfo = buildMetricInfo(metricName, layer, scope, labelMap, Optional.empty(), Optional.empty(), Optional.ofNullable(record.getName()));
            metricData.setMetric(metricInfo);
    ...
    private void metricsValuesQuery(String metricName, Layer layer, Scope scope, Map<LabelName, String> labelMap, MetricsRangeResult matrixResult) throws IOException, IllegalExpressionException {
        ...
        MetricRangeData metricData = new MetricRangeData();
        MetricInfo metricInfo = buildMetricInfo(metricName, layer, scope, labelMap, Optional.empty(), Optional.empty(), Optional.empty());
        metricData.setMetric(metricInfo);
    ...
    private void labeledMetricsValuesQuery(String metricName, Layer layer, Scope scope, Map<LabelName, String> labelMap, MetricsRangeResult matrixResult) throws IOException, IllegalExpressionException {
        ...
        List<MetricsValues> metricsValuesList = metricsQuery.readLabeledMetricsValues(metricsCondition, queryLabelList, duration);
        for (MetricsValues metricsValues : metricsValuesList) {
            MetricRangeData metricData = new MetricRangeData();
            MetricInfo metricInfo = buildMetricInfo(metricName, layer, scope, labelMap, Optional.ofNullable(relabelMap.getOrDefault(metricsValues.getLabel(), metricsValues.getLabel())), Optional.empty(), Optional.empty());
            metricData.setMetric(metricInfo);
            metricData.setValues(buildMatrixValues(duration, metricsValues));
            matrixResult.getMetricDataList().add(metricData);
        }
    ...
    private MetricInfo buildMetricInfo(String metricName, Layer layer, Scope scope, Map<LabelName, String> labelMap, Optional<String> valueLabel, Optional<String> topNEntityName, Optional<String> recordName) throws IllegalExpressionException {
        ...
        case Service: 
            ...
            if (topNEntityName.isPresent()) {
                ...
                checkLabels(labelMap, LabelName.SERVICE);
                metricInfo.getLabels().add(new LabelValuePair(LabelName.SERVICE, labelMap.get(LabelName.SERVICE)));
            }
            break;
        case ServiceInstance: 
            ...
            if (topNEntityName.isPresent()) {
                ...
                metricInfo.getLabels().add(new LabelValuePair(LabelName.SERVICE, labelMap.get(LabelName.SERVICE)));
                metricInfo.getLabels().add(new LabelValuePair(LabelName.SERVICE_INSTANCE, labelMap.get(LabelName.SERVICE_INSTANCE)));
            }
            break;
        case Endpoint: 
            ...
            if (topNEntityName.isPresent()) {
                ...
                metricInfo.getLabels().add(new LabelValuePair(LabelName.SERVICE, labelMap.get(LabelName.SERVICE)));
                metricInfo.getLabels().add(new LabelValuePair(LabelName.ENDPOINT, labelMap.get(LabelName.ENDPOINT)));
            }
            break;
        }
        return metricInfo;
