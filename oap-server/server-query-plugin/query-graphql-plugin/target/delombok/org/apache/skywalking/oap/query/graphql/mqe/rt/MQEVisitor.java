...
public class MQEVisitor extends MQEVisitorBase {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MQEVisitor.class);
    private final MetricsQuery metricsQuery;
    private final RecordsQuery recordsQuery;
    private final Entity entity;
    private final Duration duration;

    public MQEVisitor(final MetricsQuery metricsQuery, final RecordsQuery recordsQuery, final Entity entity, final Duration duration) {
        ...
        try {
            if (Column.ValueDataType.COMMON_VALUE == dataType) {
                if (ctx.parent instanceof MQEParser.TopNOPContext) {
                    ...
                    querySortMetrics(metricName, Integer.parseInt(parent.INTEGER().getText()), Order.valueOf(parent.order().getText().toUpperCase()), result);
                ...
                if (ctx.parent instanceof MQEParser.TopNOPContext) {
                    throw new IllegalExpressionException("Metric: [" + metricName + "] is labeled value, dose not support top_n query.");
                }
                if (ctx.label() != null) {
                ...
                if (ctx.parent instanceof MQEParser.TopNOPContext) {
                    ...
                    queryRecords(metricName, Integer.parseInt(parent.INTEGER().getText()), Order.valueOf(parent.order().getText().toUpperCase()), result);
                } else {
    ...
    private void queryMetrics(String metricName, Duration queryDuration, ExpressionResult result) throws IOException {
        ...
        for (int i = 0; i < times.size(); i++) {
            long retTimestamp = DurationUtils.INSTANCE.parseToDateTime(queryDuration.getStep(), times.get(i).getPoint()).getMillis();
    ...
    private void queryLabeledMetrics(String metricName, List<String> queryLabelList, Duration queryDuration, ExpressionResult result) throws IOException {
        MetricsCondition metricsCondition = new MetricsCondition();
        metricsCondition.setName(metricName);
        metricsCondition.setEntity(entity);
        List<MetricsValues> metricsValuesList = metricsQuery.readLabeledMetricsValues(metricsCondition, queryLabelList, queryDuration);
        List<PointOfTime> times = queryDuration.assembleDurationPoints();
