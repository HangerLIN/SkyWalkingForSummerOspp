...
public class UIConfigurationManagement implements GraphQLQueryResolver, GraphQLMutationResolver {
    ...
    private UITemplateManagementService getUITemplateManagementService() {
        if (uiTemplateManagementService == null) {
            this.uiTemplateManagementService = manager.find(CoreModule.NAME).provider().getService(UITemplateManagementService.class);
        }
    ...
    private UIMenuManagementService getUiMenuManagementService() {
        if (uiMenuManagementService == null) {
            this.uiMenuManagementService = manager.find(CoreModule.NAME).provider().getService(UIMenuManagementService.class);
        }
    ...
    public TemplateChangeStatus changeTemplate(DashboardSetting setting) throws IOException {
        if (!config.isEnableUpdateUITemplate()) {
            return TemplateChangeStatus.builder().status(false).id(setting.getId()).message("The dashboard update has been disabled. Check SW_ENABLE_UPDATE_UI_TEMPLATE on " + "configuration-vocabulary.md(https://skywalking.apache.org/docs/main/next/en/setup/backend/configuration-vocabulary/#configuration-vocabulary) " + "to activate it.").build();
        }
    ...
    public TemplateChangeStatus disableTemplate(String id) throws IOException {
        if (!config.isEnableUpdateUITemplate()) {
            return TemplateChangeStatus.builder().status(false).id(id).message("The dashboard disable has been disabled. Check SW_ENABLE_UPDATE_UI_TEMPLATE on " + "configuration-vocabulary.md(https://skywalking.apache.org/docs/main/next/en/setup/backend/configuration-vocabulary/#configuration-vocabulary) " + "to activate it.").build();
        }
    ...
    @lombok.Generated
    public UIConfigurationManagement(final ModuleManager manager, final GraphQLQueryConfig config) {
