...
public class OndemandLogQuery implements GraphQLQueryResolver {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OndemandLogQuery.class);
    private final MetadataQueryV2 metadataQuery;

    public PodContainers listContainers(final OndemandContainergQueryCondition condition) throws IOException {
        final ServiceInstance instance = metadataQuery.getInstance(condition.getServiceInstanceId());
    ...
    public Logs ondemandPodLogs(final OndemandLogQueryCondition condition) throws IOException {
        final ServiceInstance instance = metadataQuery.getInstance(condition.getServiceInstanceId());
        final Map<String, String> attributesMap = convertInstancePropertiesToMap(instance);
        final String ns = attributesMap.get(PropertyUtil.NAMESPACE);
        final String pod = attributesMap.get(PropertyUtil.POD);
    ...
    protected Map<String, String> convertInstancePropertiesToMap(final ServiceInstance instance) {
        ...
        final Map<String, String> attributesMap = attributes.stream().collect(Collectors.toMap(Attribute::getName, Attribute::getValue));
        if (!attributesMap.containsKey(PropertyUtil.NAMESPACE) || !attributesMap.containsKey(PropertyUtil.POD)) {
            return Collections.emptyMap();
        }
        return attributesMap;
    }

    public PodContainers listContainers(final String namespace, final String podName) {
        try (var client = new KubernetesClientBuilder().build()) {
            ...
            final var containers = spec.getContainers().stream().map(Container::getName).collect(Collectors.toList());
            if (nonNull(spec.getInitContainers())) {
                final var init = spec.getInitContainers().stream().map(Container::getName).collect(Collectors.toList());
    ...
    public Logs ondemandPodLogs(final String namespace, final String podName, final OndemandLogQueryCondition condition) {
        if (Strings.isNullOrEmpty(namespace) || Strings.isNullOrEmpty(podName)) {
        ...
        try (var client = new KubernetesClientBuilder().build()) {
            ...
            final var podLog = client.pods().inNamespace(podMetadata.getNamespace()).withName(podMetadata.getName()).inContainer(container).usingTimestamps().sinceSeconds((int) since).getLog();
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public OndemandLogQuery(final MetadataQueryV2 metadataQuery) {
