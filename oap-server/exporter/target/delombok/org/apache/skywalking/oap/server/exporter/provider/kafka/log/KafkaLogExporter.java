...
public class KafkaLogExporter extends KafkaExportProducer implements LogExportService, IConsumer<LogRecord> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(KafkaLogExporter.class);
    ...
    public void start() {
        super.getProducer();
        exportBuffer = new DataCarrier<>("KafkaLogExporter", "KafkaLogExporter", setting.getBufferChannelNum(), setting.getBufferChannelSize(), BufferStrategy.IF_POSSIBLE);
        ...
        successCounter = metricsCreator.createCounter("kafka_exporter_log_success_count", "The success number of log exported by kafka exporter.", new MetricsTag.Keys("protocol"), new MetricsTag.Values("kafka"));
        errorCounter = metricsCreator.createCounter("kafka_exporter_log_error_count", "The error number of log exported by kafka exporter", new MetricsTag.Keys("protocol"), new MetricsTag.Values("kafka"));
    }
        ...
        for (LogRecord logRecord : data) {
            if (logRecord != null) {
                try {
                    ...
                    ProducerRecord<String, Bytes> record = new ProducerRecord<>(setting.getKafkaTopicLog(), logRecord.id().build(), Bytes.wrap(logData.toByteArray()));
    ...
    private LogData transLogData(LogRecord logRecord) throws InvalidProtocolBufferException {
        ...
        if (StringUtil.isNotEmpty(logRecord.getServiceInstanceId())) {
            builder.setServiceInstance(IDManager.ServiceInstanceID.analysisId(logRecord.getServiceInstanceId()).getName());
        }
        if (StringUtil.isNotEmpty(logRecord.getEndpointId())) {
            builder.setEndpoint(IDManager.EndpointID.analysisId(logRecord.getEndpointId()).getEndpointName());
        }
