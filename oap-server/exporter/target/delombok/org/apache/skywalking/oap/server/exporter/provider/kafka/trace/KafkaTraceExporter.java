...
public class KafkaTraceExporter extends KafkaExportProducer implements TraceExportService, IConsumer<SegmentRecord> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(KafkaTraceExporter.class);
    ...
    public void start() {
        super.getProducer();
        exportBuffer = new DataCarrier<>("KafkaTraceExporter", "KafkaTraceExporter", setting.getBufferChannelNum(), setting.getBufferChannelSize(), BufferStrategy.IF_POSSIBLE);
        ...
        successCounter = metricsCreator.createCounter("kafka_exporter_trace_success_count", "The success number of traces exported by kafka exporter.", new MetricsTag.Keys("protocol"), new MetricsTag.Values("kafka"));
        errorCounter = metricsCreator.createCounter("kafka_exporter_trace_error_count", "The error number of traces exported by kafka exporter", new MetricsTag.Keys("protocol"), new MetricsTag.Values("kafka"));
    }
        ...
        for (SegmentRecord segmentRecord : data) {
            if (segmentRecord != null) {
                try {
                    ...
                    ProducerRecord<String, Bytes> record = new ProducerRecord<>(setting.getKafkaTopicTrace(), segmentObject.getTraceSegmentId(), Bytes.wrap(segmentObject.toByteArray()));
