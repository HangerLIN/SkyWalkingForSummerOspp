...
public class GRPCCallback implements AlarmCallback {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(GRPCCallback.class);
    ...
    private void sendAlarmMessages(AlarmServiceGrpc.AlarmServiceStub alarmServiceStub, List<AlarmMessage> alarmMessages, GRPCAlarmSetting alarmSetting) {
        ...
        while (!status.isDone()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Send {} alarm message to {}:{}.", alarmMessages.size(), alarmSetting.getTargetHost(), alarmSetting.getTargetPort());
            }
            if (sleepTime > 2000L) {
                log.warn("Send {} alarm message to {}:{}, wait {} milliseconds.", alarmMessages.size(), alarmSetting.getTargetHost(), alarmSetting.getTargetPort(), sleepTime);
                cycle = 2000L;
            }
        }
    }
