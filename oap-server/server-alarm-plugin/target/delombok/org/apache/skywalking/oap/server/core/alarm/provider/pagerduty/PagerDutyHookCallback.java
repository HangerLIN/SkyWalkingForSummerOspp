...
public class PagerDutyHookCallback extends HttpAlarmCallback {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PagerDutyHookCallback.class);
    ...
    public void doAlarm(List<AlarmMessage> alarmMessages) throws Exception {
        ...
        for (Map.Entry<String, List<AlarmMessage>> entry : groupedMessages.entrySet()) {
            ...
            if (setting == null || CollectionUtils.isEmpty(setting.getIntegrationKeys()) || CollectionUtils.isEmpty(messages)) {
                continue;
            }
            for (final var integrationKey : setting.getIntegrationKeys()) {
                for (final var alarmMessage : messages) {
                    try {
                        post(URI.create(PAGER_DUTY_EVENTS_API_V2_URL), getMessageBody(alarmMessage, integrationKey, setting.getTextTemplate()), Map.of());
                    } catch (Exception e) {
