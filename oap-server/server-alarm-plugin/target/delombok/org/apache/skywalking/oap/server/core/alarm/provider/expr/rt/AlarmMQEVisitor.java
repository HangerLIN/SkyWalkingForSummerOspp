...
public class AlarmMQEVisitor extends MQEVisitorBase {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(AlarmMQEVisitor.class);
    ...
    public AlarmMQEVisitor(final LinkedList<Map<String, Metrics>> metricsValues, final LocalDateTime endTime, final int maxTrendRange) {
        super(Step.MINUTE);
    ...
    private void initCommonMetricValues(String metricName, double value, long timeBucket) {
        Map<String, Double> timeValues = commonValuesMap.computeIfAbsent(metricName, v -> new HashMap<>());
    ...
    private void initLabeledMetricValues(String metricName, DataTable values, long timeBucket) {
        Map<String, DataTable> timeValues = labeledValuesMap.computeIfAbsent(metricName, v -> new HashMap<>());
    ...
    private List<MQEValues> buildLabledMqeValuesList(Map<String, DataTable> timeValues, List<String> labelValues) {
        ...
        if (CollectionUtils.isEmpty(labelValues) || labelValues.stream().allMatch(Strings::isNullOrEmpty)) {
            allLabelValues = timeValues.values().stream().flatMap(dataTable -> dataTable.keys().stream()).distinct().collect(Collectors.toList());
        } else {
