...
public class HttpUriRecognitionService implements HttpUriRecognition {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(HttpUriRecognitionService.class);
    ...
    public List<HttpUriPattern> fetchAllPatterns(final String service) {
        try {
            ...
            final HttpUriRecognitionResponse httpUriRecognitionResponse = stub.withDeadlineAfter(30, TimeUnit.SECONDS).fetchAllPatterns(HttpUriRecognitionSyncRequest.newBuilder().setService(service).setVersion(version).build());
    ...
    public void feedRawData(final String service, final List<HTTPUri> unrecognizedURIs) {
        try {
            ...
            List<HttpRawUri> rawUriList = unrecognizedURIs.stream().map(httpUri -> HttpRawUri.newBuilder().setName(httpUri.getName()).build()).collect(Collectors.toList());
            builder.addAllUnrecognizedUris(rawUriList);
            stub.withDeadlineAfter(30, TimeUnit.SECONDS).feedRawData(builder.build());
        } catch (Exception e) {
