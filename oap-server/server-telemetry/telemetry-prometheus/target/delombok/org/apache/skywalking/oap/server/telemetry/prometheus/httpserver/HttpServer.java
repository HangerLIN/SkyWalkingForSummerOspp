...
public final class HttpServer {
    ...
    public void start() throws InterruptedException {
        ...
        b.group(bossGroup, workerGroup).channel(NioServerSocketChannel.class).handler(new LoggingHandler(LogLevel.INFO)).childHandler(new HttpServerInitializer(sslCtx));
        b.bind(config.getHost(), config.getPort()).sync();
        Optional.ofNullable(sslCtx).ifPresent(HTTPDynamicSslContext::start);
        log.info("Prometheus exporter endpoint:" + (config.isSslEnabled() ? "https" : "http") + "://" + config.getHost() + ":" + config.getPort() + '/');
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public HttpServer(final PrometheusConfig config) {
        this.config = config;
    }
}
