...
public class ConsulCoordinator extends ClusterCoordinator {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ConsulCoordinator.class);
    ...
    public void registerRemote(RemoteInstance remoteInstance) throws ServiceRegisterException {
        ...
        try {
            ...
            Registration registration =  // registers with a TTL of 5 seconds
            ImmutableRegistration.builder().id(remoteInstance.getAddress().toString()).name(serviceName).address(remoteInstance.getAddress().getHost()).port(remoteInstance.getAddress().getPort()).check(Registration.RegCheck.grpc(remoteInstance.getAddress().getHost() + ":" + remoteInstance.getAddress().getPort(), 5)).build();
    ...
    public void start() {
        initHealthChecker();
        ServiceHealthCache svHealth = ServiceHealthCache.newCache(client.healthClient(), serviceName, true, QueryOptions.BLANK, 5);
                ...
                if (newValues.size() > 0) {
                    ...
                    newValues.values().forEach(serviceHealth -> {
                        if (StringUtil.isNotBlank(serviceHealth.getService().getAddress())) {
                            RemoteInstance remoteInstance = buildRemoteInstance(serviceHealth.getService().getAddress(), serviceHealth.getService().getPort());
