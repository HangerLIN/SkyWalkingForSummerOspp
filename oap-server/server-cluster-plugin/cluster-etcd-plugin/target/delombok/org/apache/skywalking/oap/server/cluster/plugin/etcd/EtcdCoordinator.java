...
public class EtcdCoordinator extends ClusterCoordinator {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(EtcdCoordinator.class);
    ...
    public EtcdCoordinator(final ModuleDefineHolder manager, final ClusterModuleEtcdConfig config) throws ModuleStartException {
        ...
        if (config.isAuthentication()) {
            builder.user(ByteSequence.from(config.getUser(), Charset.defaultCharset())).password(ByteSequence.from(config.getPassword(), Charset.defaultCharset()));
        }
    ...
    public List<RemoteInstance> queryRemoteNodes() {
        ...
        try {
            ...
            final GetResponse response = kvClient.get(serviceNameBS, GetOption.newBuilder().withPrefix(serviceNameBS).build()).get();
            response.getKvs().forEach(kv -> {
                EtcdEndpoint endpoint = GSON.fromJson(kv.getValue().toString(Charset.defaultCharset()), EtcdEndpoint.class);
    ...
    public void registerRemote(RemoteInstance remoteInstance) throws ServiceRegisterException {
        ...
        final EtcdEndpoint endpoint = new EtcdEndpoint.Builder().serviceName(serviceName).host(selfAddress.getHost()).port(selfAddress.getPort()).build();
        try {
            ...
            client.getKVClient().put(buildKey(serviceName, selfAddress, remoteInstance), instance, PutOption.newBuilder().withLeaseId(leaseID).build()).get();
    ...
    private static ByteSequence buildKey(String serviceName, Address address, RemoteInstance instance) {
        String key = new StringBuilder(serviceName).append(address.getHost()).append("_").append(instance.hashCode()).toString();
    ...
    public void start() {
        initHealthChecker();
        this.client.getWatchClient().watch(serviceNameBS, WatchOption.newBuilder().withPrefix(serviceNameBS).build(), new EtcdEventListener());
    }
