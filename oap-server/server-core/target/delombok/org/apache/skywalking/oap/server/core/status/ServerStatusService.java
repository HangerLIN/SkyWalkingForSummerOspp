...
public class ServerStatusService implements Service {
    ...
    public void bootedNow(List<ApplicationConfiguration.ModuleConfiguration> configurations, long uptime) {
        ...
        manager.find(TelemetryModule.NAME).provider().getService(MetricsCreator.class).createGauge("uptime", "oap server start up time", MetricsTag.EMPTY_KEY, MetricsTag.EMPTY_VALUE).setValue(uptime / 1000.0);
    ...
    public void rebalancedCluster(long rebalancedTime) {
        ...
        manager.find(TelemetryModule.NAME).provider().getService(MetricsCreator.class).createGauge("cluster_rebalanced_time", "oap cluster rebalanced time after scale", MetricsTag.EMPTY_KEY, MetricsTag.EMPTY_VALUE).setValue(rebalancedTime / 1000.0);
    ...
    public String dumpBootingConfigurations(String keywords4MaskingSecretsOfConfig) {
        ...
        for (ApplicationConfiguration.ModuleConfiguration configuration : configurations) {
            ...
            if (configuration.getProviders().size() == 1) {
                configList.append(moduleName).append(".provider=").append(configuration.getProviders().keySet().iterator().next()).append("\n");
            }
            configuration.getProviders().forEach((providerName, providerConfiguration) -> providerConfiguration.getProperties().forEach((key, value) -> {
                for (final String keyword : keywords) {
                    if (key.toString().toLowerCase().contains(keyword.toLowerCase())) {
                        value = "******";
                    }
                }
                configList.append(moduleName).append(".").append(providerName).append(".").append(key).append("=").append(value).append("\n");
            }));
        }
    ...
    @lombok.Generated
    public ServerStatusService(final ModuleManager manager, final CoreModuleConfig moduleConfig) {
