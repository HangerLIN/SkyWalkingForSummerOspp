...
public class TopologyQueryService implements Service {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TopologyQueryService.class);
    ...
    public Topology getGlobalTopology(final Duration duration) throws IOException {
        List<Call.CallDetail> serviceRelationServerCalls = getTopologyQueryDAO().loadServiceRelationsDetectedAtServerSide(duration);
        List<Call.CallDetail> serviceRelationClientCalls = getTopologyQueryDAO().loadServiceRelationDetectedAtClientSide(duration);
    ...
    public Topology getServiceTopology(final Duration duration, final List<String> serviceIds) throws IOException {
        List<Call.CallDetail> serviceRelationClientCalls = getTopologyQueryDAO().loadServiceRelationDetectedAtClientSide(duration, serviceIds);
        List<Call.CallDetail> serviceRelationServerCalls = getTopologyQueryDAO().loadServiceRelationsDetectedAtServerSide(duration, serviceIds);
        ServiceTopologyBuilder builder = new ServiceTopologyBuilder(moduleManager);
        ...
        if (CollectionUtils.isNotEmpty(outScopeSourceServiceIds)) {
            ...
            List<Call.CallDetail> sourceCalls = getTopologyQueryDAO().loadServiceRelationsDetectedAtServerSide(duration, outScopeSourceServiceIds);
     ...
     */
    public ServiceInstanceTopology getServiceInstanceTopology(final String clientServiceId, final String serverServiceId, final Duration duration) throws IOException {
        List<Call.CallDetail> serviceInstanceRelationClientCalls = getTopologyQueryDAO().loadInstanceRelationDetectedAtClientSide(clientServiceId, serverServiceId, duration);
        List<Call.CallDetail> serviceInstanceRelationServerCalls = getTopologyQueryDAO().loadInstanceRelationDetectedAtServerSide(clientServiceId, serverServiceId, duration);
    ...
    @Deprecated
    public Topology getEndpointTopology(final Duration duration, final String endpointId) throws IOException {
        List<Call.CallDetail> serverSideCalls = getTopologyQueryDAO().loadEndpointRelation(duration, endpointId);
    ...
    public EndpointTopology getEndpointDependencies(final Duration duration, final String endpointId) throws IOException {
        List<Call.CallDetail> serverSideCalls = getTopologyQueryDAO().loadEndpointRelation(duration, endpointId);
