...
public class MetricsStreamProcessor implements StreamProcessor<Metrics> {
     ...
     */
    public void create(ModuleDefineHolder moduleDefineHolder, StreamDefinition stream, Class<? extends Metrics> meterClass) throws StorageException {
        this.create(moduleDefineHolder, stream, meterClass, MetricStreamKind.MAL);
    }

    private void create(ModuleDefineHolder moduleDefineHolder, StreamDefinition stream, Class<? extends Metrics> metricsClass, MetricStreamKind kind) throws StorageException {
        ...
        if (supportDownSampling) {
            if (configService.shouldToHour()) {
                Model model = modelSetter.add(metricsClass, stream.getScopeId(), new Storage(stream.getName(), timeRelativeID, DownSampling.Hour));
                hourPersistentWorker = downSamplingWorker(moduleDefineHolder, metricsDAO, model, supportUpdate, kind);
            }
            if (configService.shouldToDay()) {
                Model model = modelSetter.add(metricsClass, stream.getScopeId(), new Storage(stream.getName(), timeRelativeID, DownSampling.Day));
        ...
        Model model = modelSetter.add(metricsClass, stream.getScopeId(), new Storage(stream.getName(), timeRelativeID, DownSampling.Minute));
        MetricsPersistentWorker minutePersistentWorker = minutePersistentWorker(moduleDefineHolder, metricsDAO, model, transWorker, supportUpdate, kind);
        ...
        MetricsAggregateWorker aggregateWorker = new MetricsAggregateWorker(moduleDefineHolder, remoteWorker, stream.getName(), l1FlushPeriod, kind);
     ...
     */
    private MetricsPersistentWorker minutePersistentWorker(ModuleDefineHolder moduleDefineHolder, IMetricsDAO metricsDAO, Model model, MetricsTransWorker transWorker, boolean supportUpdate, MetricStreamKind kind) {
        ...
        MetricsPersistentWorker minutePersistentWorker = new MetricsPersistentWorker(moduleDefineHolder, model, metricsDAO, alarmNotifyWorker, exportWorker, transWorker, supportUpdate, storageSessionTimeout, metricsDataTTL, kind);
        persistentWorkers.add(minutePersistentWorker);
        return minutePersistentWorker;
    }

    private MetricsPersistentWorker downSamplingWorker(ModuleDefineHolder moduleDefineHolder, IMetricsDAO metricsDAO, Model model, boolean supportUpdate, MetricStreamKind kind) {
        MetricsPersistentWorker persistentWorker = new MetricsPersistentWorker(moduleDefineHolder, model, metricsDAO, supportUpdate, storageSessionTimeout, metricsDataTTL, kind);
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public void setL1FlushPeriod(final long l1FlushPeriod) {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public void setStorageSessionTimeout(final long storageSessionTimeout) {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public void setMetricsDataTTL(final int metricsDataTTL) {
