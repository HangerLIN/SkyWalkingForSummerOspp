...
public class ProcessTopologyBuilder {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ProcessTopologyBuilder.class);
    ...
    ProcessTopology build(List<Call.CallDetail> clientCalls, List<Call.CallDetail> serverCalls) throws Exception {
        ...
        final Set<String> sourceProcessIdList = Stream.concat(clientCalls.stream(), serverCalls.stream()).map(Call.CallDetail::getSource).collect(Collectors.toSet());
        final Set<String> destProcessIdList = Stream.concat(clientCalls.stream(), serverCalls.stream()).map(Call.CallDetail::getTarget).collect(Collectors.toSet());
        sourceProcessIdList.addAll(destProcessIdList);
        // query all traffic data
        final Map<String, ProcessNode> nodes = this.metricsDAO.multiGet(this.processTrafficModel, Stream.concat(sourceProcessIdList.stream(), destProcessIdList.stream()).distinct().map(processId -> {
