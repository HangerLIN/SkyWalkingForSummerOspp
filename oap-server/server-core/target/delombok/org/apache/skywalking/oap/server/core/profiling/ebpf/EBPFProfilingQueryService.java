...
public class EBPFProfilingQueryService implements Service {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(EBPFProfilingQueryService.class);
    ...
    private IEBPFProfilingScheduleDAO getScheduleDAO() {
        if (scheduleDAO == null) {
            this.scheduleDAO = moduleManager.find(StorageModule.NAME).provider().getService(IEBPFProfilingScheduleDAO.class);
        }
        return scheduleDAO;
    }
    ...
    private EBPFProfilingAnalyzer getProfilingAnalyzer() {
        if (profilingAnalyzer == null) {
            this.profilingAnalyzer = new EBPFProfilingAnalyzer(moduleManager, config.getMaxDurationOfQueryEBPFProfilingData(), config.getMaxThreadCountOfQueryEBPFProfilingData());
        }
    ...
    public IServiceLabelDAO getServiceLabelDAO() {
        if (serviceLabelDAO == null) {
            serviceLabelDAO = moduleManager.find(StorageModule.NAME).provider().getService(IServiceLabelDAO.class);
        }
    ...
    public EBPFProfilingTaskPrepare queryPrepareCreateEBPFProfilingTaskData(String serviceId) throws IOException {
        ...
        final long processesCount = getMetadataQueryDAO().getProcessCount(serviceId, ProfilingSupportStatus.SUPPORT_EBPF_PROFILING, TimeBucket.getTimeBucket(startTimestamp, DownSampling.Minute), TimeBucket.getTimeBucket(endTimestamp, DownSampling.Minute));
        if (processesCount <= 0) {
    ...
    public List<EBPFProfilingSchedule> queryEBPFProfilingSchedules(String taskId) throws Exception {
        ...
        if (CollectionUtils.isNotEmpty(schedules)) {
            ...
            final List<Metrics> processMetrics = schedules.stream().map(EBPFProfilingSchedule::getProcessId).distinct().map(processId -> {
            ...
            final Map<String, Process> processMap = processes.stream().map(t -> (ProcessTraffic) t).collect(Collectors.toMap(m -> m.id().build(), this::convertProcess));
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public EBPFProfilingQueryService(final ModuleManager moduleManager, final CoreModuleConfig config, final StorageModels storageModels) {
