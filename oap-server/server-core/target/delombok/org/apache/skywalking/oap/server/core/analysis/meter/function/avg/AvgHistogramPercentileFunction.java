...
public abstract class AvgHistogramPercentileFunction extends Meter implements AcceptableValue<PercentileArgument>, MultiIntValuesHolder {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(AvgHistogramPercentileFunction.class);
    ...
    public void accept(final MeterEntity entity, final PercentileArgument value) {
        if (dataset.size() > 0) {
            if (!value.getBucketedValues().isCompatible(dataset)) {
                throw new IllegalArgumentException("Incompatible BucketedValues [" + value + "] for current PercentileFunction[" + dataset + "]");
            }
        }
        ...
        if (ranks.size() > 0) {
            if (ranks.size() != value.getRanks().length) {
                throw new IllegalArgumentException("Incompatible ranks size = [" + value.getRanks().length + "] for current PercentileFunction[" + ranks.size() + "]");
            } else {
                for (final int rank : value.getRanks()) {
                    if (!ranks.include(rank)) {
                        throw new IllegalArgumentException("Rank " + rank + " doesn\'t exist in the previous ranks " + ranks);
                    }
                }
            }
        } else {
    ...
    public boolean combine(final Metrics metrics) {
        ...
        if (this.ranks.size() > 0) {
            ...
            if (this.ranks.size() != ranksOfThat.size()) {
                log.warn("Incompatible ranks size = [{}}] for current PercentileFunction[{}]", ranksOfThat.size(), this.ranks.size());
                return true;
            } else {
    ...
    public void calculate() {
        if (!isCalculated) {
            ...
            }).collect(groupingBy(Tuple2::_1, mapping(Tuple2::_2, Collector.of(DataTable::new, (dt, key) -> {
                String v;
                if (key.contains(":")) {
                    int index = key.lastIndexOf(":");
            ...
            }, DataTable::append)))).forEach((group, subDataset) -> {
                long total;
    ...
    public int[] getValues() {
        return percentileValues.sortedValues(Comparator.comparingInt(Integer::parseInt)).stream().flatMapToInt(l -> IntStream.of(l.intValue())).toArray();
    }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public void setPercentileValues(final DataTable percentileValues) {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public void setSummation(final DataTable summation) {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public void setCount(final DataTable count) {
        this.count = count;
    }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public void setRanks(final IntList ranks) {
        this.ranks = ranks;
    }
}
