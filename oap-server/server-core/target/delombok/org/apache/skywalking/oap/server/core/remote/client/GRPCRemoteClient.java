...
public class GRPCRemoteClient implements RemoteClient {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(GRPCRemoteClient.class);
    ...
    public GRPCRemoteClient(final ModuleDefineHolder moduleDefineHolder, final Address address, final int channelSize, final int bufferSize, final int remoteTimeout, final SslContext sslContext) {
        this.address = address;
        this.channelSize = channelSize;
        this.bufferSize = bufferSize;
        this.remoteTimeout = remoteTimeout;
        this.sslContext = sslContext;
        remoteOutCounter = moduleDefineHolder.find(TelemetryModule.NAME).provider().getService(MetricsCreator.class).createCounter("remote_out_count", "The number(client side) of inside remote inside aggregate rpc.", new MetricsTag.Keys("dest", "self"), new MetricsTag.Values(address.toString(), "N"));
        remoteOutErrorCounter = moduleDefineHolder.find(TelemetryModule.NAME).provider().getService(MetricsCreator.class).createCounter("remote_out_error_count", "The error number(client side) of inside remote inside aggregate rpc.", new MetricsTag.Keys("dest", "self"), new MetricsTag.Values(address.toString(), "N"));
    }
    ...
    private StreamObserver<RemoteMessage> createStreamObserver() {
        ...
        while (concurrentStreamObserverNumber.get() > 10) {
            ...
            if (sleepTotalMillis > 60000) {
                log.warn("Remote client [{}] block times over 60 seconds. Current streaming number {}", address, concurrentStreamObserverNumber.get());
                // Reset sleepTotalMillis to avoid too many warn logs.
                sleepTotalMillis = 0;
            }
        }
        final StreamObserver<RemoteMessage> remoteMessageStreamObserver = getStub().withDeadlineAfter(remoteTimeout, TimeUnit.SECONDS).call(new StreamObserver<Empty>() {
            @Override
