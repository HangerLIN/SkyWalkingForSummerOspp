...
public class MetricsAggregateWorker extends AbstractWorker<Metrics> {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MetricsAggregateWorker.class);
    ...
    MetricsAggregateWorker(ModuleDefineHolder moduleDefineHolder, AbstractWorker<Metrics> nextWorker, String modelName, long l1FlushPeriod, MetricStreamKind kind) {
        ...
        this.dataCarrier = new DataCarrier<>("MetricsAggregateWorker." + modelName, name, queueChannelSize, queueBufferSize, BufferStrategy.IF_POSSIBLE);
        ...
        abandonCounter = metricsCreator.createCounter("metrics_aggregator_abandon", "The abandon number of rows received in aggregation", new MetricsTag.Keys("metricName", "level", "dimensionality"), new MetricsTag.Values(modelName, "1", "minute"));
        aggregationCounter = metricsCreator.createCounter("metrics_aggregation", "The number of rows in aggregation", new MetricsTag.Keys("metricName", "level", "dimensionality"), new MetricsTag.Values(modelName, "1", "minute"));
    ...
    private void flush() {
        ...
        if (currentTime - lastSendTime > l1FlushPeriod) {
            mergeDataCache.read().forEach(data -> {
                nextWorker.in(data);
            });
