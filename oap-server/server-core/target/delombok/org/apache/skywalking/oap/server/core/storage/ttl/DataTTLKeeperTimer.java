...
public enum DataTTLKeeperTimer {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(DataTTLKeeperTimer.class);
    ...
    private void delete() {
        ...
        if (CollectionUtils.isNotEmpty(remoteInstances) && !remoteInstances.get(0).getAddress().isSelf()) {
            log.info("The selected first getAddress is {}. The remove stage is skipped.", remoteInstances.get(0).toString());
            return;
        }
    ...
    private void execute(Model model) {
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Model {}, is record? {}. RecordDataTTL {}, MetricsDataTTL {}", model.getName(), model.isRecord(), moduleConfig.getRecordDataTTL(), moduleConfig.getMetricsDataTTL());
            }
