...
public class JDBCSQLExecutor {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JDBCSQLExecutor.class);
    ...
    protected <T extends StorageData> List<StorageData> getByIDs(JDBCClient h2Client, String modelName, List<String> ids, StorageBuilder<T> storageBuilder) throws Exception {
        ...
        for (var table : tables) {
            final var sql = new SQLBuilder("SELECT * FROM " + table + " WHERE id in ").append(ids.stream().map(it -> "?").collect(Collectors.joining(",", "(", ")")));
    ...
    protected <T extends StorageData> StorageData getByID(JDBCClient h2Client, String modelName, String id, StorageBuilder<T> storageBuilder) {
        try {
            final var tables = getModelTables(h2Client, modelName);
            for (var table : tables) {
                final var result = h2Client.executeQuery("SELECT * FROM " + table + " WHERE id = ?", resultSet -> toStorageData(resultSet, modelName, storageBuilder), TableHelper.generateId(modelName, id));
                if (result != null) {
                    return result;
                }
            }
    ...
    protected <T extends StorageData> SQLExecutor getInsertExecutor(Model model, T metrics, long timeBucket, StorageBuilder<T> storageBuilder, Convert2Storage<Map<String, Object>> converter, SessionCacheCallback callback) throws IOException {
        ...
        SQLExecutor sqlExecutor = buildInsertExecutor(model, metrics, timeBucket, mainEntity, callback);
        ...
        for (final var additionalTable : model.getSqlDBModelExtension().getAdditionalTables().values()) {
            ...
            List<SQLExecutor> additionalSQLExecutors = buildAdditionalInsertExecutor(model, additionalTable.getName(), additionalTable.getColumns(), metrics, timeBucket, additionalEntity, callback);
    ...
    private <T extends StorageData> SQLExecutor buildInsertExecutor(Model model, T metrics, long timeBucket, Map<String, Object> objectMap, SessionCacheCallback onCompleteCallback) {
        ...
        final var columnNames = Stream.concat(Stream.of(JDBCTableInstaller.ID_COLUMN, JDBCTableInstaller.TABLE_COLUMN), columns.stream().map(ModelColumn::getColumnName).map(ColumnName::getStorageName)).collect(Collectors.toList());
        ...
        final var param = Stream.concat(Stream.of(TableHelper.generateId(model, metrics.id().build()), model.getName()), columns.stream().map(ModelColumn::getColumnName).map(ColumnName::getName).map(objectMap::get).map(it -> {
    ...
    private <T extends StorageData> List<SQLExecutor> buildAdditionalInsertExecutor(Model model, String tableName, List<ModelColumn> columns, T metrics, long timeBucket, Map<String, Object> objectMap, SessionCacheCallback callback) {
        ...
        final var sqlBuilder = new SQLBuilder("INSERT INTO ").append(TableHelper.getTable(tableName, timeBucket)).append(" VALUES ");
    ...
    private static ArrayList<String> getModelTables(JDBCClient h2Client, String modelName) throws Exception {
        ...
        try (
            var connection = h2Client.getConnection();
            var resultSet = connection.getMetaData().getTables(connection.getCatalog(), null, tableNamePattern, new String[] {"TABLE"})) {
