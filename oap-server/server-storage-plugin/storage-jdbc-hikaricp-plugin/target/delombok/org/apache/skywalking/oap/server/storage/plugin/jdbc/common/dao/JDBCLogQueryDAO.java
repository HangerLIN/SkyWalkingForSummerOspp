...
public class JDBCLogQueryDAO implements ILogQueryDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JDBCLogQueryDAO.class);
    ...
    @Override
    public Logs queryLogs(String serviceId, String serviceInstanceId, String endpointId, TraceScopeCondition relatedTrace, Order queryOrder, int from, int limit, final Duration duration, final List<Tag> tags, final List<String> keywordsOfContent, final List<String> excludingKeywordsOfContent) {
        try {
            ...
            if (tags != null && !searchableTagKeys.containsAll(tags.stream().map(Tag::getKey).collect(toSet()))) {
                log.warn("Searching tags that are not searchable: {}", tags.stream().map(Tag::getKey).filter(not(searchableTagKeys::contains)).collect(toSet()));
                return new Logs();
            }
            final var tables = tableHelper.getTablesForRead(LogRecord.INDEX_NAME, duration.getStartTimeBucket(), duration.getEndTimeBucket());
            final var logs = new ArrayList<Log>();
            for (final var table : tables) {
                final var sqlAndParameters = buildSQL(serviceId, serviceInstanceId, endpointId, relatedTrace, queryOrder, from, limit, duration, tags, keywordsOfContent, excludingKeywordsOfContent, table);
                logs.addAll(jdbcClient.executeQuery(sqlAndParameters.sql(), this::parseResults, sqlAndParameters.parameters()));
            }
            final var comparator = Order.ASC.equals(queryOrder) ? comparing(Log::getTimestamp) : comparing(Log::getTimestamp).reversed();
            return new Logs(logs.stream().sorted(comparator).skip(from).limit(limit).collect(toList()));
    ...
    protected SQLAndParameters buildSQL(String serviceId, String serviceInstanceId, String endpointId, TraceScopeCondition relatedTrace, Order queryOrder, int from, int limit, final Duration duration, final List<Tag> tags, final List<String> keywordsOfContent, final List<String> excludingKeywordsOfContent, final String table) {
        ...
        sql.append(" order by ").append(TIMESTAMP).append(" ").append(Order.DES.equals(queryOrder) ? "desc" : "asc");
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public JDBCLogQueryDAO(final JDBCClient jdbcClient, final ModuleManager manager, final TableHelper tableHelper) {
