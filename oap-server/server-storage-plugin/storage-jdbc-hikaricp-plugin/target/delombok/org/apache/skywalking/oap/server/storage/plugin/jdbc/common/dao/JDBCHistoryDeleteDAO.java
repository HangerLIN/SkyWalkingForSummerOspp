...
public class JDBCHistoryDeleteDAO implements IHistoryDeleteDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JDBCHistoryDeleteDAO.class);
    ...
    public void deleteHistory(Model model, String timeBucketColumnName, int ttl) {
        try {
            ...
            log.info("Deleting history data, ttl: {}, now: {}. Keep [{}, {}]", ttl, clock.millis(), startTimeBucket, endTimeBucket);
            ...
            if (deadline <= lastSuccessDeadline) {
                if (log.isDebugEnabled()) {
                    log.debug("The deadline {} is less than the last success deadline {}, skip deleting history data", deadline, lastSuccessDeadline);
                }
                return;
            }
            ...
            try (
                var conn = jdbcClient.getConnection();
                var result = conn.getMetaData().getTables(conn.getCatalog(), null, tableName + "%", new String[] {"TABLE"})) {
                while (result.next()) {
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public JDBCHistoryDeleteDAO(final JDBCClient jdbcClient, final TableHelper tableHelper, final JDBCTableInstaller modelInstaller, final Clock clock) {
