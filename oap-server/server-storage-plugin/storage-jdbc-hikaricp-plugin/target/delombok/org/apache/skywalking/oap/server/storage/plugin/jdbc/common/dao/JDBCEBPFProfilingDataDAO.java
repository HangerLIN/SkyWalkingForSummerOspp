...
public class JDBCEBPFProfilingDataDAO implements IEBPFProfilingDataDAO {
    ...
    public List<EBPFProfilingDataRecord> queryData(List<String> scheduleIdList, long beginTime, long endTime) {
        try {
            final var tables = tableHelper.getTablesForRead(EBPFProfilingDataRecord.INDEX_NAME, TimeBucket.getTimeBucket(beginTime, DownSampling.Day), TimeBucket.getTimeBucket(endTime, DownSampling.Day));
            ...
            for (final var table : tables) {
                ...
                results.addAll(jdbcClient.executeQuery(sqlAndParameters.sql(), this::buildDataList, sqlAndParameters.parameters()));
            }
            return results;
    ...
    protected SQLAndParameters buildSQL(final List<String> scheduleIdList, final long beginTime, final long endTime, final String table) {
    ...
    private <T> void appendConditions(StringBuilder conditionSql, List<Object> condition, String field, List<T> data) {
        ...
        conditionSql.append(field).append(" in ").append(data.stream().map(it -> "?").collect(joining(", ", "(", ")")));
        condition.addAll(data);
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public JDBCEBPFProfilingDataDAO(final JDBCClient jdbcClient, final TableHelper tableHelper) {
