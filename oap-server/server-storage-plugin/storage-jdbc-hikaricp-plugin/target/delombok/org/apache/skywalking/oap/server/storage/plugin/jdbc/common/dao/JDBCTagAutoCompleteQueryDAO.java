...
public class JDBCTagAutoCompleteQueryDAO implements ITagAutoCompleteQueryDAO {
    ...
    @Override
    public Set<String> queryTagAutocompleteKeys(final TagType tagType, final int limit, final Duration duration) {
        try {
            final var tables = tableHelper.getTablesForRead(TagAutocompleteData.INDEX_NAME, duration.getStartTimeBucket(), duration.getEndTimeBucket());
    ...
    protected SQLAndParameters buildSQLForQueryKeys(TagType tagType, int limit, Duration duration, String table) {
        ...
        sql.append("select distinct ").append(TagAutocompleteData.TAG_KEY).append(" from ").append(table).append(" where ");
        sql.append(" 1=1 ");
    ...
    @Override
    public Set<String> queryTagAutocompleteValues(final TagType tagType, final String tagKey, final int limit, final Duration duration) {
        try {
            final var tables = tableHelper.getTablesForRead(TagAutocompleteData.INDEX_NAME, duration.getStartTimeBucket(), duration.getEndTimeBucket());
            final var results = new HashSet<String>();
            for (String table : tables) {
    ...
    private void appendTagAutocompleteCondition(final TagType tagType, final Duration duration, final StringBuilder sql, final List<Object> condition) {
    ...
    @lombok.Generated
    public JDBCTagAutoCompleteQueryDAO(final JDBCClient jdbcClient, final TableHelper tableHelper) {
