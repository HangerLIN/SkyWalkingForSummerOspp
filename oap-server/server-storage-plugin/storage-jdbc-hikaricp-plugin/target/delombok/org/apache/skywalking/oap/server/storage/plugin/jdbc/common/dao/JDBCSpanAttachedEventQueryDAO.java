...
public class JDBCSpanAttachedEventQueryDAO implements ISpanAttachedEventQueryDAO {
    ...
    public List<SpanAttachedEventRecord> querySpanAttachedEvents(SpanAttachedEventTraceType type, List<String> traceIds) {
        try {
            ...
            return results.stream().sorted(comparing(SpanAttachedEventRecord::getStartTimeSecond).thenComparing(SpanAttachedEventRecord::getStartTimeNanos)).collect(toList());
    ...
    private static SQLAndParameters buildSQL(SpanAttachedEventTraceType type, List<String> traceIds, String table) {
        ...
        sql.append(traceIds.stream().map(it -> "?").collect(joining(",", "(", ")")));
        parameters.addAll(traceIds);
        sql.append(" and ").append(SpanAttachedEventRecord.TRACE_REF_TYPE).append(" = ?");
        parameters.add(type.value());
        sql.append(" order by ").append(SpanAttachedEventRecord.START_TIME_SECOND).append(",").append(SpanAttachedEventRecord.START_TIME_NANOS).append(" ASC ");
        return new SQLAndParameters(sql.toString(), parameters);
    }

    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public JDBCSpanAttachedEventQueryDAO(final JDBCClient jdbcClient, final TableHelper tableHelper) {
