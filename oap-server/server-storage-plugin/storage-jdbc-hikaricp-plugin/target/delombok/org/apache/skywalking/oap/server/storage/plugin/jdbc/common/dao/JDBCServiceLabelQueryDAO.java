...
public class JDBCServiceLabelQueryDAO implements IServiceLabelDAO {
    ...
    public List<String> queryAllLabels(String serviceId) {
        try {
            ...
            for (String table : tables) {
                ...
                sql.append("select " + ServiceLabelRecord.LABEL + " from ").append(table).append(" where ").append(JDBCTableInstaller.TABLE_COLUMN).append(" = ?").append(" and ").append(ServiceLabelRecord.SERVICE_ID).append(" = ?");
                condition.add(ServiceLabelRecord.INDEX_NAME);
                condition.add(serviceId);
                results.addAll(jdbcClient.executeQuery(sql.toString(), this::parseLabels, condition.toArray(new Object[0])));
            }
            return results;
    ...
    @lombok.Generated
    public JDBCServiceLabelQueryDAO(final JDBCClient jdbcClient, final TableHelper tableHelper) {
