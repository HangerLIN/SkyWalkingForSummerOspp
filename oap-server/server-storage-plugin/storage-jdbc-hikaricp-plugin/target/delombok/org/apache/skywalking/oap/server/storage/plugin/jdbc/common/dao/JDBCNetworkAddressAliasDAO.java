...
public class JDBCNetworkAddressAliasDAO extends JDBCSQLExecutor implements INetworkAddressAliasDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JDBCNetworkAddressAliasDAO.class);
    ...
    public List<NetworkAddressAlias> loadLastUpdate(long lastUpdateTime) {
        try {
            ...
            for (final var table : tables) {
                final var sql = new StringBuilder().append("select * from ").append(table).append(" where ").append(JDBCTableInstaller.TABLE_COLUMN).append(" = ? ").append(" and ").append(NetworkAddressAlias.LAST_UPDATE_TIME_BUCKET).append(">?");
                results.addAll(jdbcClient.executeQuery(sql.toString(), resultSet -> {
                    ...
                    do {
                        networkAddressAlias = (NetworkAddressAlias) toStorageData(resultSet, NetworkAddressAlias.INDEX_NAME, new NetworkAddressAlias.Builder());
