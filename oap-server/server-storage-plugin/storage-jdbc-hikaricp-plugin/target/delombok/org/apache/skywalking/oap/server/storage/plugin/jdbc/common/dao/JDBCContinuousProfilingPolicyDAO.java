...
public class JDBCContinuousProfilingPolicyDAO extends JDBCSQLExecutor implements IContinuousProfilingPolicyDAO {
    ...
    public void savePolicy(ContinuousProfilingPolicy policy) throws IOException {
        ...
        if (CollectionUtils.isNotEmpty(existingPolicy)) {
            ...
            sqlExecutor = getInsertExecutor(model, policy, 0, new ContinuousProfilingPolicy.Builder(), new HashMapConverter.ToStorage(), null);
        }
    ...
    public List<ContinuousProfilingPolicy> queryPolicies(List<String> serviceIdList) throws IOException {
        try {
            ...
            StringBuilder whereQuery = new StringBuilder().append(" where ").append(JDBCTableInstaller.TABLE_COLUMN).append(" = ?").append(" and ").append(ContinuousProfilingPolicy.SERVICE_ID).append(" in ").append(serviceIdList.stream().map(s -> "?").collect(Collectors.joining(",", "(", ")")));
    ...
    @lombok.Generated
    public JDBCContinuousProfilingPolicyDAO(final JDBCClient jdbcClient, final TableHelper tableHelper) {
