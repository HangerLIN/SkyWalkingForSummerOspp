...
public class JDBCTraceQueryDAO implements ITraceQueryDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JDBCTraceQueryDAO.class);
    private final ModuleManager manager;
    private final JDBCClient jdbcClient;
    private final TableHelper tableHelper;
    private SearchableTracesTagsWatcher searchableTagKeys;
    private static String DETAIL_SELECT_QUERY = "select " + SegmentRecord.SEGMENT_ID + ", " + SegmentRecord.TRACE_ID + ", " + SegmentRecord.ENDPOINT_ID + ", " + SegmentRecord.SERVICE_ID + ", " + SegmentRecord.SERVICE_INSTANCE_ID + ", " + SegmentRecord.START_TIME + ", " + SegmentRecord.LATENCY + ", " + SegmentRecord.IS_ERROR + ", " + SegmentRecord.DATA_BINARY;

    @Override
    public TraceBrief queryBasicTraces(Duration duration, long minDuration, long maxDuration, String serviceId, String serviceInstanceId, String endpointId, String traceId, int limit, int from, TraceState traceState, QueryOrder queryOrder, final List<Tag> tags) throws IOException {
        try {
            if (searchableTagKeys == null) {
                final ConfigService configService = manager.find(CoreModule.NAME).provider().getService(ConfigService.class);
            ...
            if (tags != null && !searchableTagKeys.getSearchableTags().containsAll(tags.stream().map(Tag::getKey).collect(toSet()))) {
                log.warn("Searching tags that are not searchable: {}", tags.stream().map(Tag::getKey).filter(not(searchableTagKeys.getSearchableTags()::contains)).collect(toSet()));
            ...
            final var tables = startSecondTB > 0 && endSecondTB > 0 ? tableHelper.getTablesForRead(SegmentRecord.INDEX_NAME, startSecondTB, endSecondTB) : tableHelper.getTablesWithinTTL(SegmentRecord.INDEX_NAME);
             ...
             */
                ...
                jdbcClient.executeQuery("select " + SegmentRecord.SEGMENT_ID + ", " + SegmentRecord.START_TIME + ", " + SegmentRecord.ENDPOINT_ID + ", " + SegmentRecord.LATENCY + ", " + SegmentRecord.IS_ERROR + ", " + SegmentRecord.TRACE_ID + " " + sql, resultSet -> {
                    while (resultSet.next()) {
                        ...
                        basicTrace.getEndpointNames().add(IDManager.EndpointID.analysisId(resultSet.getString(SegmentRecord.ENDPOINT_ID)).getEndpointName());
    ...
    public List<SegmentRecord> queryByTraceId(String traceId) throws IOException {
        try {
            ...
            for (String table : tables) {
                jdbcClient.executeQuery(DETAIL_SELECT_QUERY + " from " + table + " where " + JDBCTableInstaller.TABLE_COLUMN + " = ? and " + SegmentRecord.TRACE_ID + " = ?", resultSet -> {
    ...
    public List<SegmentRecord> queryBySegmentIdList(List<String> segmentIdList) throws IOException {
        try {
            ...
            for (String table : tables) {
                jdbcClient.executeQuery(DETAIL_SELECT_QUERY + " from " + table + " where " + JDBCTableInstaller.TABLE_COLUMN + " = ? and " + SegmentRecord.SEGMENT_ID + " in " + segmentIdList.stream().map(it -> "?").collect(Collectors.joining(",", "(", ")")), resultSet -> {
                    segmentRecords.addAll(buildRecords(resultSet));
                    return null;
                }, conditions.toArray());
            }
    ...
    public List<SegmentRecord> queryByTraceIdWithInstanceId(List<String> traceIdList, List<String> instanceIdList) throws IOException {
        try {
            ...
            for (String table : tables) {
                jdbcClient.executeQuery(DETAIL_SELECT_QUERY + " from " + table + " where " + JDBCTableInstaller.TABLE_COLUMN + " = ? and " + SegmentRecord.TRACE_ID + " in " + traceIdList.stream().map(it -> "?").collect(Collectors.joining(",", "(", ") and ")) + SegmentRecord.SERVICE_INSTANCE_ID + " in " + instanceIdList.stream().map(it -> "?").collect(Collectors.joining(",", "(", ")")), resultSet -> {
                    segmentRecords.addAll(buildRecords(resultSet));
                    return null;
                }, conditions.toArray());
            }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public JDBCTraceQueryDAO(final ModuleManager manager, final JDBCClient jdbcClient, final TableHelper tableHelper) {
