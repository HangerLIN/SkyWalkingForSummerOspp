...
public class JDBCEBPFProfilingTaskDAO extends JDBCSQLExecutor implements IEBPFProfilingTaskDAO {
    ...
    protected SQLAndParameters buildSQLForQueryTasksByServices(final List<String> serviceIdList, EBPFProfilingTriggerType triggerType, final long taskStartTime, final long latestUpdateTime, final String table) {
        ...
        if (taskStartTime > 0) {
            appendCondition(conditionSql, parameters, EBPFProfilingTaskRecord.START_TIME, ">=", taskStartTime);
        }
        if (latestUpdateTime > 0) {
            appendCondition(conditionSql, parameters, EBPFProfilingTaskRecord.LAST_UPDATE_TIME, ">", latestUpdateTime);
        }
        if (triggerType != null) {
            appendCondition(conditionSql, parameters, EBPFProfilingTaskRecord.TRIGGER_TYPE, "=", triggerType.value());
        }
    ...
    public List<EBPFProfilingTaskRecord> queryTasksByServices(List<String> serviceIdList, EBPFProfilingTriggerType triggerType, long taskStartTime, long latestUpdateTime) throws IOException {
        try {
            ...
            for (final var table : tables) {
                ...
                results.addAll(jdbcClient.executeQuery(sqlAndParameters.sql(), this::buildTasks, sqlAndParameters.parameters()));
            }
            return results;
    ...
    public List<EBPFProfilingTaskRecord> queryTasksByTargets(String serviceId, String serviceInstanceId, List<EBPFProfilingTargetType> targetTypes, EBPFProfilingTriggerType triggerType, long taskStartTime, long latestUpdateTime) throws IOException {
        try {
            ...
            final var tables = tableHelper.getTablesWithinTTL(EBPFProfilingTaskRecord.INDEX_NAME);
            for (final var table : tables) {
                final var sqlAndParameters = buildSQLForQueryTasksByTargets(serviceId, serviceInstanceId, targetTypes, triggerType, taskStartTime, latestUpdateTime, table);
                results.addAll(jdbcClient.executeQuery(sqlAndParameters.sql(), this::buildTasks, sqlAndParameters.parameters()));
            }
            return results;
        } catch (final java.lang.Throwable $ex) {
    ...
    public List<EBPFProfilingTaskRecord> getTaskRecord(String id) throws IOException {
        try {
            ...
            final var tables = tableHelper.getTablesWithinTTL(EBPFProfilingTaskRecord.INDEX_NAME);
            for (final var table : tables) {
                String sql = "select * from " + table + " where " + JDBCTableInstaller.TABLE_COLUMN + " = ?" + EBPFProfilingTaskRecord.LOGICAL_ID + " = ?";
                results.addAll(jdbcClient.executeQuery(sql, this::buildTasks, EBPFProfilingTaskRecord.INDEX_NAME, id));
            }
            return results;
        } catch (final java.lang.Throwable $ex) {
            throw lombok.Lombok.sneakyThrow($ex);
    ...
    protected SQLAndParameters buildSQLForQueryTasksByTargets(final String serviceId, final String serviceInstanceId, final List<EBPFProfilingTargetType> targetTypes, EBPFProfilingTriggerType triggerType, final long taskStartTime, final long latestUpdateTime, final String table) {
        final var sql = new StringBuilder();
        final var parameters = new ArrayList<>();
        ...
        appendListCondition(conditions, parameters, EBPFProfilingTaskRecord.TARGET_TYPE, targetTypes.stream().map(EBPFProfilingTargetType::value).collect(Collectors.toList()));
        if (taskStartTime > 0) {
            appendCondition(conditions, parameters, EBPFProfilingTaskRecord.START_TIME, ">=", taskStartTime);
        }
        if (latestUpdateTime > 0) {
            appendCondition(conditions, parameters, EBPFProfilingTaskRecord.LAST_UPDATE_TIME, ">", latestUpdateTime);
        }
        if (triggerType != null) {
            appendCondition(conditions, parameters, EBPFProfilingTaskRecord.TRIGGER_TYPE, "=", triggerType.value());
        }
    ...
    @lombok.Generated
    public JDBCEBPFProfilingTaskDAO(final JDBCClient jdbcClient, final TableHelper tableHelper) {
