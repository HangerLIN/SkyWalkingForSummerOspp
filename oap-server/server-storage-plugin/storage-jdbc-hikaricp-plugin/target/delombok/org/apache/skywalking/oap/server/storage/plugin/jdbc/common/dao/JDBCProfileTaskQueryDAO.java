...
public class JDBCProfileTaskQueryDAO implements IProfileTaskQueryDAO {
    ...
    public List<ProfileTask> getTaskList(String serviceId, String endpointName, Long startTimeBucket, Long endTimeBucket, Integer limit) {
        try {
            ...
            final var tables = startTimeBucket == null || endTimeBucket == null ? tableHelper.getTablesWithinTTL(ProfileTaskRecord.INDEX_NAME) : tableHelper.getTablesForRead(ProfileTaskRecord.INDEX_NAME, startTimeBucket, endTimeBucket);
            for (final var table : tables) {
                final var condition = new ArrayList<>(4);
                final var sql = new StringBuilder().append("select * from ").append(table).append(" where ").append(JDBCTableInstaller.TABLE_COLUMN).append(" = ?");
            ...
            return limit == null ? results : results.stream().limit(limit).collect(toList());
    ...
    public ProfileTask getById(String id) {
        try {
            ...
            for (String table : tables) {
                ...
                sql.append("select * from ").append(table).append(" where ").append(JDBCTableInstaller.TABLE_COLUMN).append(" = ? ").append(" and ").append(ProfileTaskRecord.TASK_ID + "=? LIMIT 1");
                condition.add(ProfileTaskRecord.INDEX_NAME);
                condition.add(id);
                final var r = jdbcClient.executeQuery(sql.toString(), resultSet -> {
                    if (resultSet.next()) {
    ...
    private ProfileTask parseTask(ResultSet data) throws SQLException {
        return ProfileTask.builder().id(data.getString(ProfileTaskRecord.TASK_ID)).serviceId(data.getString(ProfileTaskRecord.SERVICE_ID)).endpointName(data.getString(ProfileTaskRecord.ENDPOINT_NAME)).startTime(data.getLong(ProfileTaskRecord.START_TIME)).createTime(data.getLong(ProfileTaskRecord.CREATE_TIME)).duration(data.getInt(ProfileTaskRecord.DURATION)).minDurationThreshold(data.getInt(ProfileTaskRecord.MIN_DURATION_THRESHOLD)).dumpPeriod(data.getInt(ProfileTaskRecord.DUMP_PERIOD)).maxSamplingCount(data.getInt(ProfileTaskRecord.MAX_SAMPLING_COUNT)).build();
    }
}
