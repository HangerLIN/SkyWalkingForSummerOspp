...
public class JDBCEventQueryDAO implements IEventQueryDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JDBCEventQueryDAO.class);
    ...
    public Events queryEvents(final EventQueryCondition condition) throws Exception {
        ...
        final var comparator = Order.DES.equals(queryOrder) ? comparing(org.apache.skywalking.oap.server.core.query.type.event.Event::getStartTime).reversed() : comparing(org.apache.skywalking.oap.server.core.query.type.event.Event::getStartTime);
        return new Events(events.stream().sorted(comparator).skip(page.getFrom()).limit(page.getLimit()).collect(toList()));
    }

    @Override
    public Events queryEvents(List<EventQueryCondition> conditions) throws Exception {
        final var conditionsParametersPair = conditions.stream().map(this::buildQuery).collect(toList());
        final var parameters = conditionsParametersPair.stream().map(Tuple2::_2).reduce(Stream.empty(), Stream::concat).toArray();
        final var whereClause = conditionsParametersPair.stream().map(Tuple2::_1).map(it -> it.collect(joining(" and "))).collect(joining(" or ", " where ", ""));
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public JDBCEventQueryDAO(final JDBCClient jdbcClient, final TableHelper tableHelper) {
