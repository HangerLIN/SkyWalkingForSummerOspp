...
public class JDBCMetadataQueryDAO implements IMetadataQueryDAO {
    ...
    public List<Service> listServices() {
        try {
            ...
            for (final var table : tables) {
                ...
                results.addAll(jdbcClient.executeQuery(sqlAndParameters.sql(), this::buildServices, sqlAndParameters.parameters()));
            }
            return results.stream().limit(metadataQueryMaxSize).collect(toList());
    ...
    protected SQLAndParameters buildSQLForListServices(String table) {
        ...
        sql.append("select * from ").append(table).append(" where ").append(JDBCTableInstaller.TABLE_COLUMN).append(" = ? ");
    ...
    public List<ServiceInstance> listInstances(Duration duration, String serviceId) {
        try {
            ...
            for (String table : tables) {
                ...
                results.addAll(jdbcClient.executeQuery(sqlAndParameters.sql(), this::buildInstances, sqlAndParameters.parameters()));
            }
            return results.stream().limit(metadataQueryMaxSize).collect(toList());
        } catch (final java.lang.Throwable $ex) {
            throw lombok.Lombok.sneakyThrow($ex);
    ...
    public List<Endpoint> findEndpoint(String keyword, String serviceId, int limit) {
        try {
            ...
            for (String table : tables) {
                ...
                results.addAll(jdbcClient.executeQuery(sql.toString(), resultSet -> {
    ...
    public List<Process> listProcesses(String serviceId, ProfilingSupportStatus supportStatus, long lastPingStartTimeBucket, long lastPingEndTimeBucket) {
        try {
            ...
            if (lastPingStartTimeBucket > 0 && lastPingEndTimeBucket > 0) {
                tables = tableHelper.getTablesForRead(ProcessTraffic.INDEX_NAME, lastPingStartTimeBucket, lastPingEndTimeBucket);
            } else {
            ...
            for (String table : tables) {
                ...
                results.addAll(jdbcClient.executeQuery(sqlAndParameters.sql(), this::buildProcesses, sqlAndParameters.parameters()));
            }
            return results.stream().limit(metadataQueryMaxSize).collect(toList());
        } catch (final java.lang.Throwable $ex) {
            throw lombok.Lombok.sneakyThrow($ex);
    ...
    protected SQLAndParameters buildSQLForListProcesses(final String serviceId, final ProfilingSupportStatus supportStatus, final long lastPingStartTimeBucket, final long lastPingEndTimeBucket, final String table) {
        final var sql = new StringBuilder();
    ...
    public List<Process> listProcesses(String serviceInstanceId, Duration duration, boolean includeVirtual) {
        try {
            final var tables = tableHelper.getTablesForRead(ProcessTraffic.INDEX_NAME, duration.getStartTimeBucket(), duration.getEndTimeBucket());
            final var results = new ArrayList<Process>();
            for (String table : tables) {
    ...
    public List<Process> listProcesses(String agentId) {
        try {
            ...
            for (String table : tables) {
                ...
                sql.append(" limit ").append(metadataQueryMaxSize);
                results.addAll(jdbcClient.executeQuery(sql.toString(), this::buildProcesses, condition.toArray(new Object[0])));
            }
            return results.stream().limit(metadataQueryMaxSize).collect(toList());
        } catch (final java.lang.Throwable $ex) {
            throw lombok.Lombok.sneakyThrow($ex);
    ...
    public long getProcessCount(String serviceId, ProfilingSupportStatus profilingSupportStatus, long lastPingStartTimeBucket, long lastPingEndTimeBucket) {
        try {
            final var tables = tableHelper.getTablesForRead(ProcessTraffic.INDEX_NAME, lastPingStartTimeBucket, lastPingEndTimeBucket);
            long total = 0;
            for (String table : tables) {
                StringBuilder sql = new StringBuilder();
                List<Object> condition = new ArrayList<>(5);
                ...
                sql.append(" where ").append(JDBCTableInstaller.TABLE_COLUMN).append(" = ? ");
                condition.add(ProcessTraffic.INDEX_NAME);
                appendProcessWhereQuery(sql, condition, serviceId, null, null, profilingSupportStatus, lastPingStartTimeBucket, lastPingEndTimeBucket, false);
    ...
    private List<ServiceInstance> buildInstances(ResultSet resultSet) throws SQLException {
        ...
        while (resultSet.next()) {
            final var instanceTraffic = new InstanceTraffic.Builder().storage2Entity(JDBCEntityConverters.toEntity(resultSet));
    ...
    private void appendProcessWhereQuery(StringBuilder sql, List<Object> condition, String serviceId, String instanceId, String agentId, final ProfilingSupportStatus profilingSupportStatus, final long lastPingStartTimeBucket, final long lastPingEndTimeBucket, boolean includeVirtual) {
