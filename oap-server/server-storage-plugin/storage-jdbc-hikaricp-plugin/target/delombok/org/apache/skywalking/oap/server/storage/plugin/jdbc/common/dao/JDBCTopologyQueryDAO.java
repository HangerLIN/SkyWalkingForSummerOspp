...
public class JDBCTopologyQueryDAO implements ITopologyQueryDAO {
    ...
    public List<Call.CallDetail> loadServiceRelationsDetectedAtServerSide(Duration duration, List<String> serviceIds) throws IOException {
        return loadServiceCalls(ServiceRelationServerSideMetrics.INDEX_NAME, duration, ServiceRelationServerSideMetrics.SOURCE_SERVICE_ID, ServiceRelationServerSideMetrics.DEST_SERVICE_ID, serviceIds, DetectPoint.SERVER);
    }
    ...
    public List<Call.CallDetail> loadServiceRelationDetectedAtClientSide(Duration duration, List<String> serviceIds) throws IOException {
        return loadServiceCalls(ServiceRelationClientSideMetrics.INDEX_NAME, duration, ServiceRelationClientSideMetrics.SOURCE_SERVICE_ID, ServiceRelationClientSideMetrics.DEST_SERVICE_ID, serviceIds, DetectPoint.CLIENT);
    }

    @Override
    public List<Call.CallDetail> loadServiceRelationsDetectedAtServerSide(Duration duration) throws IOException {
        return loadServiceCalls(ServiceRelationServerSideMetrics.INDEX_NAME, duration, ServiceRelationServerSideMetrics.SOURCE_SERVICE_ID, ServiceRelationServerSideMetrics.DEST_SERVICE_ID, new ArrayList<>(0), DetectPoint.SERVER);
    }

    @Override
    public List<Call.CallDetail> loadServiceRelationDetectedAtClientSide(Duration duration) throws IOException {
        return loadServiceCalls(ServiceRelationClientSideMetrics.INDEX_NAME, duration, ServiceRelationClientSideMetrics.SOURCE_SERVICE_ID, ServiceRelationClientSideMetrics.DEST_SERVICE_ID, new ArrayList<>(0), DetectPoint.CLIENT);
    }
    ...
    public List<Call.CallDetail> loadInstanceRelationDetectedAtServerSide(String clientServiceId, String serverServiceId, Duration duration) throws IOException {
        return loadServiceInstanceCalls(ServiceInstanceRelationServerSideMetrics.INDEX_NAME, duration, ServiceInstanceRelationServerSideMetrics.SOURCE_SERVICE_ID, ServiceInstanceRelationServerSideMetrics.DEST_SERVICE_ID, clientServiceId, serverServiceId, DetectPoint.SERVER);
    }
    ...
    public List<Call.CallDetail> loadInstanceRelationDetectedAtClientSide(String clientServiceId, String serverServiceId, Duration duration) throws IOException {
        return loadServiceInstanceCalls(ServiceInstanceRelationClientSideMetrics.INDEX_NAME, duration, ServiceInstanceRelationClientSideMetrics.SOURCE_SERVICE_ID, ServiceInstanceRelationClientSideMetrics.DEST_SERVICE_ID, clientServiceId, serverServiceId, DetectPoint.CLIENT);
    }

    @Override
    public List<Call.CallDetail> loadEndpointRelation(Duration duration, String destEndpointId) throws IOException {
        List<Call.CallDetail> calls = loadEndpointFromSide(EndpointRelationServerSideMetrics.INDEX_NAME, duration, EndpointRelationServerSideMetrics.SOURCE_ENDPOINT, EndpointRelationServerSideMetrics.DEST_ENDPOINT, destEndpointId, false);
        calls.addAll(loadEndpointFromSide(EndpointRelationServerSideMetrics.INDEX_NAME, duration, EndpointRelationServerSideMetrics.SOURCE_ENDPOINT, EndpointRelationServerSideMetrics.DEST_ENDPOINT, destEndpointId, true));
        return calls;
    }
    ...
    private List<Call.CallDetail> loadServiceCalls(String tableName, Duration duration, String sourceCName, String destCName, List<String> serviceIds, DetectPoint detectPoint) {
        try {
            final var tables = tableHelper.getTablesForRead(tableName, duration.getStartTimeBucket(), duration.getEndTimeBucket());
            ...
            for (String table : tables) {
                ...
                jdbcClient.executeQuery("select " + Metrics.ENTITY_ID + ", " + ServiceRelationServerSideMetrics.COMPONENT_IDS + " from " + table + " where " + JDBCTableInstaller.TABLE_COLUMN + " = ? and " + Metrics.TIME_BUCKET + ">= ? and " + Metrics.TIME_BUCKET + "<=? " + serviceIdMatchSql + " group by " + Metrics.ENTITY_ID + "," + ServiceRelationServerSideMetrics.COMPONENT_IDS, resultSet -> {
    ...
    private List<Call.CallDetail> loadServiceInstanceCalls(String tableName, Duration duration, String sourceCName, String descCName, String sourceServiceId, String destServiceId, DetectPoint detectPoint) throws IOException {
        try {
            ...
            for (String table : tables) {
                ...
                jdbcClient.executeQuery("select " + Metrics.ENTITY_ID + " from " + table + " where " + JDBCTableInstaller.TABLE_COLUMN + " = ? and " + Metrics.TIME_BUCKET + ">= ? and " + Metrics.TIME_BUCKET + "<=? " + serviceIdMatchSql + " group by " + Metrics.ENTITY_ID, resultSet -> {
    ...
    private List<Call.CallDetail> loadEndpointFromSide(String tableName, Duration duration, String sourceCName, String destCName, String id, boolean isSourceId) throws IOException {
        try {
            ...
            for (String table : tables) {
                ...
                conditions[2] = duration.getEndTimeBucket();
                conditions[3] = id;
                jdbcClient.executeQuery("select " + Metrics.ENTITY_ID + " from " + table + " where " + JDBCTableInstaller.TABLE_COLUMN + " = ? and " + Metrics.TIME_BUCKET + ">= ? and " + Metrics.TIME_BUCKET + "<=? and " + (isSourceId ? sourceCName : destCName) + "=?" + " group by " + Metrics.ENTITY_ID, resultSet -> {
    ...
    private List<Call.CallDetail> loadProcessFromSide(Duration duration, String instanceId, DetectPoint detectPoint) throws IOException {
        try {
            ...
            for (String table : tables) {
                ...
                jdbcClient.executeQuery("select " + Metrics.ENTITY_ID + ", " + ProcessRelationServerSideMetrics.COMPONENT_ID + " from " + table + " where " + JDBCTableInstaller.TABLE_COLUMN + " = ? and " + Metrics.TIME_BUCKET + ">= ? and " + Metrics.TIME_BUCKET + "<=? and " + ProcessRelationClientSideMetrics.SERVICE_INSTANCE_ID + "=?" + " group by " + Metrics.ENTITY_ID + ", " + ProcessRelationServerSideMetrics.COMPONENT_ID, resultSet -> {
    ...
    private void buildServiceCalls(ResultSet resultSet, List<Call.CallDetail> calls, DetectPoint detectPoint) throws SQLException {
        while (resultSet.next()) {
            ...
            final IntList componentIds = new IntList(resultSet.getString(ServiceRelationServerSideMetrics.COMPONENT_IDS));
    ...
    @lombok.Generated
    public JDBCTopologyQueryDAO(final JDBCClient jdbcClient, final TableHelper tableHelper) {
