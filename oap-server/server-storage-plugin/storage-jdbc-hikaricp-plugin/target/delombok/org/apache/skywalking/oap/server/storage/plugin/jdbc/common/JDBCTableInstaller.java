...
public class JDBCTableInstaller extends ModelInstaller {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JDBCTableInstaller.class);
    ...
    public boolean isExists(Model model) {
        try {
            ...
            final var isAnyColumnNotCreated = model.getColumns().stream().map(ModelColumn::getColumnName).map(ColumnName::getStorageName).anyMatch(not(databaseColumns::contains));
    ...
    public void createOrUpdateTableIndexes(String table, List<ModelColumn> columns, boolean isAdditionalTable) throws SQLException {
        ...
        if (isAdditionalTable) {
            ...
            if (!jdbcClient.indexExists(table, index)) {
                executeSQL(new SQLBuilder("CREATE INDEX ").append(index).append(" ON ").append(table).append("(").append(ID_COLUMN).append(")"));
            }
        }
        if (!isAdditionalTable) {
            final var index = "idx_" + Math.abs((table + "_" + JDBCTableInstaller.TABLE_COLUMN).hashCode());
            if (!jdbcClient.indexExists(table, index)) {
                executeSQL(new SQLBuilder("CREATE INDEX ").append(index).append(" ON ").append(table).append("(").append(JDBCTableInstaller.TABLE_COLUMN).append(")"));
            }
        }
        final var columnsMissingIndex = columns.stream().filter(ModelColumn::shouldIndex).filter(it -> it.getLength() < 256).map(ModelColumn::getColumnName).map(ColumnName::getStorageName).collect(toList());
        for (var column : columnsMissingIndex) {
            final var index = "idx_" + Math.abs((table + "_" + column).hashCode());
            if (!jdbcClient.indexExists(table, index)) {
                executeSQL(new SQLBuilder("CREATE INDEX ").append(index).append(" ON ").append(table).append("(").append(column).append(")"));
            }
        }
        final var columnNames = columns.stream().map(ModelColumn::getColumnName).map(ColumnName::getStorageName).collect(toSet());
        for (final var modelColumn : columns) {
            for (final var compositeIndex : modelColumn.getSqlDatabaseExtension().getIndices()) {
                ...
                executeSQL(new SQLBuilder("CREATE INDEX ").append(index).append(" ON ").append(table).append(multiColumns.stream().collect(joining(", ", " (", ")"))));
            }
        }
    }
    ...
    private void updateTable(String table, List<ModelColumn> columns) throws SQLException {
        final var alterSqls = columns.stream().map(this::getColumnDefinition).map(definition -> "ALTER TABLE " + table + " ADD COLUMN " + definition + "; ").collect(toList());
    ...
    private void createTable(String table, List<ModelColumn> columns, boolean isAdditionalTable) throws SQLException {
        ...
        columns.stream().map(this::getColumnDefinition).collect(toCollection(() -> columnDefinitions));
        final var sql = new SQLBuilder("CREATE TABLE IF NOT EXISTS " + table).append(columnDefinitions.stream().collect(joining(", ", " (", ");")));
        executeSQL(sql);
    }
}
