...
public class BanyanDBUITemplateManagementDAO extends AbstractBanyanDBDAO implements UITemplateManagementDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(BanyanDBUITemplateManagementDAO.class);
    ...
    public List<DashboardConfiguration> getAllTemplates(Boolean includingDisabled) throws IOException {
        ...
        return propertyList.stream().map(p -> fromEntity(parse(p))).filter(conf -> includingDisabled || !conf.isDisabled()).collect(Collectors.toList());
    }
    ...
    public Property applyAll(UITemplate uiTemplate) {
        return Property.create(GROUP, UITemplate.INDEX_NAME, uiTemplate.id().build()).addTag(TagAndValue.newStringTag(UITemplate.CONFIGURATION, uiTemplate.getConfiguration())).addTag(TagAndValue.newLongTag(UITemplate.DISABLED, uiTemplate.getDisabled())).addTag(TagAndValue.newLongTag(UITemplate.UPDATE_TIME, uiTemplate.getUpdateTime())).build();
    }
    ...
    public Property applyStatus(UITemplate uiTemplate) {
        return Property.create(GROUP, UITemplate.INDEX_NAME, uiTemplate.id().build()).addTag(TagAndValue.newLongTag(UITemplate.DISABLED, uiTemplate.getDisabled())).addTag(TagAndValue.newLongTag(UITemplate.UPDATE_TIME, uiTemplate.getUpdateTime())).build();
    }
    ...
    public Property applyConfiguration(UITemplate uiTemplate) {
        return Property.create(GROUP, UITemplate.INDEX_NAME, uiTemplate.id().build()).addTag(TagAndValue.newStringTag(UITemplate.CONFIGURATION, uiTemplate.getConfiguration())).addTag(TagAndValue.newLongTag(UITemplate.UPDATE_TIME, uiTemplate.getUpdateTime())).build();
    }
}
