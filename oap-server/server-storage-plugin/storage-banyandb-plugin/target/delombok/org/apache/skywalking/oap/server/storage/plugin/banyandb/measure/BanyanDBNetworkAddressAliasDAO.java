...
public class BanyanDBNetworkAddressAliasDAO extends AbstractBanyanDBDAO implements INetworkAddressAliasDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(BanyanDBNetworkAddressAliasDAO.class);
    private final NetworkAddressAlias.Builder builder = new NetworkAddressAlias.Builder();
    private MetadataRegistry.Schema schema;
    private static final Set<String> TAGS = ImmutableSet.of(NetworkAddressAlias.ADDRESS, NetworkAddressAlias.LAST_UPDATE_TIME_BUCKET, NetworkAddressAlias.REPRESENT_SERVICE_ID, NetworkAddressAlias.REPRESENT_SERVICE_INSTANCE_ID);
    ...
    public List<NetworkAddressAlias> loadLastUpdate(long timeBucket) {
        try {
            MeasureQueryResponse resp = query(NetworkAddressAlias.INDEX_NAME, TAGS, Collections.emptySet(), new QueryBuilder<MeasureQuery>() {
                @Override
             ...
             */
            return resp.getDataPoints().stream().map(point -> builder.storage2Entity(new StorageToMeasure(getSchema(), point))).collect(Collectors.toList());
        } catch (IOException e) {
