...
public class MetricsEsDAO extends EsDAO implements IMetricsDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MetricsEsDAO.class);
    ...
    public List<Metrics> multiGet(Model model, List<Metrics> metrics) {
        ...
        if (model.isTimeRelativeID()) {
            ...
            groupIndices.forEach((tableName, metricList) -> {
                List<String> ids = metricList.stream().map(item -> IndexController.INSTANCE.generateDocId(model, item.id().build())).collect(Collectors.toList());
    ...
    @Override
    public boolean isExpiredCache(final Model model, final Metrics cachedValue, final long currentTimeMillis, final int ttl) {
        final long metricTimestamp = TimeBucket.getTimestamp(cachedValue.getTimeBucket(), model.getDownsampling());
