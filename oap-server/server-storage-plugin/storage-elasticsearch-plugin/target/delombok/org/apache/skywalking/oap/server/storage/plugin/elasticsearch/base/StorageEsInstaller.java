...
public class StorageEsInstaller extends ModelInstaller {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(StorageEsInstaller.class);
    ...
    public boolean isExists(Model model) throws StorageException {
        ...
        if (exist) {
            structures.putStructure(tableName, template.get().getMappings(), template.get().getSettings());
    ...
    private void createNormalTable(Model model) throws StorageException {
        ...
        if (!esClient.isExistsIndex(tableName)) {
            ...
            if (appendMapping.getProperties() != null && !appendMapping.getProperties().isEmpty()) {
                ...
                log.info("update {} index mapping finished, isAcknowledged: {}, append mapping: {}", tableName, isAcknowledged, appendMapping);
                if (!isAcknowledged) {
            ...
            if (!structures.compareIndexSetting(tableName, settings)) {
                log.warn("index {} settings configuration has been updated to {}, please remove it before OAP starts", tableName, settings);
            }
        }
    }
    ...
    private void createTimeSeriesTable(Model model) throws StorageException {
        ...
        try {
            ...
            shouldUpdateTemplate = shouldUpdateTemplate || !structures.containsMapping(tableName, mapping) || !structures.compareIndexSetting(tableName, settings);
            ...
            if (esClient.isExistsIndex(indexName)) {
                ...
                //needs to update settings
                if (!structures.compareIndexSetting(tableName, settings)) {
                    log.info("index template {} settings configuration has been updated to {}, it will applied on new index", tableName, settings);
                }
            } else {
    ...
    protected Map<String, Object> createSetting(Model model) throws StorageException {
        ...
        indexSettings.put("number_of_replicas", model.isSuperDataset() ? Integer.toString(config.getSuperDatasetIndexReplicasNumber()) : Integer.toString(config.getIndexReplicasNumber()));
        indexSettings.put("number_of_shards", model.isSuperDataset() ? Integer.toString(config.getIndexShardsNumber() * config.getSuperDatasetIndexShardsFactor()) : Integer.toString(config.getIndexShardsNumber()));
    ...
    private Map getAnalyzerSettingByColumn(Model model) throws StorageException {
        ...
        for (final ModelColumn column : analyzerTypes) {
            ...
            AnalyzerSetting setting = AnalyzerSetting.Generator.getGenerator(column.getElasticSearchExtension().getAnalyzer()).getGenerateFunc().generate(config);
    ...
    private Map getAnalyzerSetting4MergedIndex(Model model) throws StorageException {
        AnalyzerSetting setting = AnalyzerSetting.Generator.getGenerator(ElasticSearch.MatchQuery.AnalyzerType.OAP_ANALYZER).getGenerateFunc().generate(config);
    ...
    protected Mappings createMapping(Model model) {
        ...
        if ((model.isMetric() && !config.isLogicSharding()) || (config.isLogicSharding() && IndexController.INSTANCE.isFunctionMetric(model))) {
            Map<String, Object> column = new HashMap<>();
            column.put("type", "keyword");
