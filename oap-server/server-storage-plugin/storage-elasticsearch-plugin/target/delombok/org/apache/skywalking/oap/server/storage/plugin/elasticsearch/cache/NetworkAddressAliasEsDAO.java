...
public class NetworkAddressAliasEsDAO extends EsDAO implements INetworkAddressAliasDAO {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(NetworkAddressAliasEsDAO.class);
    ...
    public List<NetworkAddressAlias> loadLastUpdate(long timeBucketInMinute) {
        final String index = IndexController.LogicIndicesRegister.getPhysicalTableName(NetworkAddressAlias.INDEX_NAME);
        try {
            ...
            query.must(Query.range(NetworkAddressAlias.LAST_UPDATE_TIME_BUCKET).gte(timeBucketInMinute));
            final var search = Search.builder().query(query).size(batchSize).build();
            final var builder = new NetworkAddressAlias.Builder();
            final var scroller = ElasticSearchScroller.<NetworkAddressAlias>builder().client(getClient()).search(search).index(index).queryMaxSize(resultWindowMaxSize).resultConverter(searchHit -> builder.storage2Entity(new ElasticSearchConverter.ToEntity(NetworkAddressAlias.INDEX_NAME, searchHit.getSource()))).build();
